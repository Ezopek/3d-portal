...
initiatives:
  ...
  - id: 23
    name: 'Spoolman Filament Profile Estimates (Material-Default + Exact-Override Policy)'
    status: 'planning'
    started: '2026-06-07'
    epics: 'E35 (Spoolman-backed Orca filament-profile estimate policy — portal-owned profile-selection policy layered over the Init 20 resolver/bundle: material-type default Orca filament profiles + optional per-Spoolman-filament exact overrides + a classified EstimateProfileSource {exact_filament_mapping, default_material_profile, unavailable_no_profile} returned to the read APIs/UI. Spoolman stays SoT for inventory + generic material; the portal owns estimate-profile policy. Generic material default resolves automatically for new colors (bounded default matrix STL × offer/process × compatible material-default profile); exact per-filament overrides are an explicit operator-managed exception path, NOT precomputed. Stories: 35.1 policy model + file-backed ProfilePolicyStore + precedence resolver (shipped first — pure/standalone foundation); 35.2 resolver/bundle integration (select Orca filament profile by policy before bundle materialization; two generic PLA colors share bundle, PLA Matt exact override differs); 35.3 estimate ingest/read API + DTO source metadata; 35.4 admin policy management surface; 35.5 user-facing estimate UI source labels (exact/default/unavailable honesty); 35.6 bounded default-matrix backfill + enqueue guardrails (dry-run counts). Preserves Init 20 (stl_hash, bundle_hash) dedupe/cache invariants. Source SCP: sprint-change-proposal-2026-06-07-spoolman-filament-profile-estimates.md.'
---
...
##### Story PROFILE-PUBLISH-1 — Compile a usable PrintProfileOffer's ProfileChain into a real resolver bundle + prove live slicing (G-PUBLISH backend bridge; FR21-SELECTOR-1 backend half, NFR21-PROVENANCE-1, NFR21-OBS-1, NFR21-AUTH-1, NFR21-DETERMINISM-1; Decision AR)

> ✅ **READY-FOR-DEV (2026-06-06).** Spec authored: `_bmad-output/implementation-artifacts/profile-publish-1-compile-offer-chain-resolver-bridge.md` (status `ready-for-dev`; sprint-status row `profile-publish-1-compile-offer-chain-resolver-bridge: ready-for-dev`). This realizes the **backend half** of the long-deferred **G-PUBLISH** gate named in `sprint-change-proposal-2026-06-06-epic33-profile-model-correction.md` § 6 + Decision AN (provisionally "PROFILE-OFFER-2"; renamed **PROFILE-PUBLISH-1**). G-ARCH/G-DEVGO/G-DATA are satisfied; dev-story may start. G-DATA: offer `561d9ea327e143da9bfcc1031cda8077` + catalog STL `282d26c1660c41b30d15b293b5c92bfe494ab62d76350009ceba55e714774b7f`.

**Realizes:** the bridge that compiles a `usable` `PrintProfileOffer`'s embedded `ProfileChain` (three library blocks: machine + process + filament) into a **real resolver-produced, byte-pinned Orca bundle** with a real `bundle_hash`, persisted into the existing **append-only** bundle store, then enqueues **ONE** live slice/estimate over **ONE** operator-designated catalog STL — proving the offer/chain layer reaches the real resolver/worker/`bundle_hash`/estimate path end-to-end. New `slicer/profile_publish.py` orchestrator + a chain-addressed resolve entry that **reuses the `resolve()` tail verbatim** (single SoT); additive publish-state on the existing offer sidecar (atomic, no DB); `POST /api/admin/profiles/offers/{offer_id}/publish` + `.../unpublish`; admin-only estimate observation reusing `GET /api/estimates`; minimal admin publish/observe FE (may split to a follow-on).

**Decision confirmed by G-ARCH (Decision AR):** the publish bridge mechanism — **option (a)** intent-coordinate publish (writes an `intents/<printer>/<material>/<tier>.json` triple, **rejected as default**: forces a synthetic grid coordinate + risks colliding with the grid projection the member selector consumes) vs **option (b)** chain-addressed resolve-tail (**accepted**: resolves the chain directly from its three library block bodies through the existing resolve pipeline, never reading/writing the `intents/`/`system/` grid trees, never minting a grid coordinate). Gemini architecture review APPROVE (2026-06-06) confirmed option (b); ACs remain written against (b).

**Depends on:** PROFILE-OFFER-1 (done — the offer/chain data-model + admin CRUD + dry validation) + PROFILE-LIB-1 + 33.1 + 33.2 (done) + Init 20 resolver/bundle/estimate machinery.

**Coexistence (SCP § 4/§ 3.7):** the member-facing selector does **NOT** change — it keeps consuming the transitional 33.1/33.2 grid projection unchanged. Turning a published offer into a member-facing choice is the explicit, separately-gated follow-on **PROFILE-PUBLISH-2**.

**Out of scope:** member selector / member offer surface / NO-422 contract change (→ PROFILE-PUBLISH-2); any write to the grid `intents/`/`system/` trees or synthetic grid coordinate (Decision AR option b keeps the publish disjoint); change to the 33.1 read, the 33.2 import, `compatibility.py`, the `bundle_hash` input order, or the append-only store **layout**; N×M editor; raw Orca JSON viewer; Spoolman read/write/mutation or concrete-filament override publication; Alembic/DB; broad member order/request flow.

**Escalation vs LIB-1/OFFER-1:** this is the **first E33 slice to legitimately touch the resolve / bundle / slice path**, so — unlike the deploy-clean LIB-1/OFFER-1 — **SW-DEPLOY-1 IS triggered** (`apps/api/app/modules/slicer/**` change + the slicer worker consumes the newly-published bundle): overlay `portal-slicer-worker:0.1.0` rebuild + worker smoke, fatal-on-fail. External review is high-stakes (Gemini default, Codex fallback for resolver/slice-path + data-integrity adjacency).

**Gates:** **G-ARCH SATISFIED** (Gemini architecture review APPROVE confirmed Decision AR option b — chain-addressed resolve-tail); **G-DEVGO SATISFIED** (operator: "możemy lecieć z PUBLISH" — first write-bearing E33 story touching the resolve/bundle/slice path); **G-PUBLISH** (**REALIZED — backend half** by this story; no longer "deferred forever" — it is the active next E33 slice; the member-facing half remains deferred to PROFILE-PUBLISH-2); **G-DATA SATISFIED** (offer `561d9ea327e143da9bfcc1031cda8077` + catalog STL `282d26c1660c41b30d15b293b5c92bfe494ab62d76350009ceba55e714774b7f` on `.190`; synthetic substitution forbidden for the live smoke; unit/integration tests run against bench fixtures + a stubbed arq pool); **SW-DEPLOY-1** (TRIGGERED — deploy gate); **G-SMOKE** (live `.190` LIVE-SLICING smoke — deploy-GO precondition, AC-13; NOT authorized/run by this card).
...
| FR | Epic | Story | Notes |
|---|---|---|---|
| FR23-POLICY-1 | E35 | 35.1 | Portal-owned profile-selection policy model + file-backed store (material defaults + per-filament overrides). |
| FR23-PRECEDENCE-1 | E35 | 35.1 | Pure precedence resolver: exact override > material default > unavailable. |
| FR23-RESOLVER-1 | E35 | 35.2 | Resolve the Orca filament profile by policy before bundle materialization. |
| FR23-SNAPSHOT-MAP-1 | E35 | 35.2/35.3 | Map the Init 19 cached Spoolman snapshot → generic material + stable `spoolman_filament_ref`. |
| FR23-ESTIMATE-API-1 | E35 | 35.3 | Estimate ingest/read APIs accept/derive the selected filament; return source + selected material/ref/name. |
| FR23-ADMIN-1 | E35 | 35.4 | Admin policy management surface (list Spoolman materials + known Orca profiles; get/update material defaults + per-filament overrides). |
| FR23-UI-LABEL-1 | E35 | 35.5 | User-facing estimate UI labels exact vs default-fallback vs unavailable distinctly. |
| FR23-BACKFILL-1 | E35 | 35.6 | Bounded default-matrix enqueue/backfill hooks + guardrails; exclude concrete overrides unless explicitly requested. |

### Epic List

| Epic | Name | Status | Stories |
|---|---|---|---|
| E35 | Spoolman Filament Profile Estimate Policy | ✅ done | 35.1, 35.2, 35.3, 35.4, 35.5, 35.6 |

#### Epic E35 — Spoolman Filament Profile Estimate Policy

**Goal:** estimates select the Orca filament profile from Spoolman-backed generic-material defaults plus optional concrete-filament overrides, so new normal filaments estimate automatically while unusual filaments can be pinned by an admin — with honest exact/default/unavailable labelling and the Init 20 cache invariants preserved.

##### Story 35.1 — Profile-selection policy model + store + precedence resolver (FR23-POLICY-1, FR23-PRECEDENCE-1, NFR23-STABLE-KEY-1, NFR23-CACHE-INVARIANT-1, NFR23-DETERMINISM-1; Decision AS)
✅ shipped 2026-06-07.

##### Story 35.2 — Resolve Orca filament profile by policy before bundle materialization (FR23-RESOLVER-1, FR23-SNAPSHOT-MAP-1, NFR23-CACHE-INVARIANT-1, NFR23-OBS-1; Decision AS)
✅ shipped 2026-06-07.

##### Story 35.3 — Estimate ingest/read API + DTO source metadata (FR23-ESTIMATE-API-1, NFR23-HONESTY-1, NFR23-NO-BLOCK-1, NFR23-OBS-1)
✅ shipped 2026-06-07.

##### Story 35.4 — Admin policy management surface (FR23-ADMIN-1)
✅ shipped 2026-06-10.

##### Story 35.5 — User-facing estimate UI source labels (FR23-UI-LABEL-1, NFR23-HONESTY-1, NFR23-NO-BLOCK-1)
✅ shipped 2026-06-10.

##### Story 35.6 — Bounded default-matrix backfill + enqueue guardrails (FR23-BACKFILL-1, NFR23-QUEUE-BOUND-1)
✅ shipped 2026-06-10.

## Initiative 24 — Member-Facing Published Profile Offer Surface (PROFILE-PUBLISH-2)

**Status:** 🚧 planning (started 2026-06-13). Source SCP: `sprint-change-proposal-2026-06-13-profile-publish-2-member-offer-surface.md` (status `proposed`). Single epic **E36**. Architecture: `architecture.md` § Initiative 24 (Decision AT). Realizes the named **PROFILE-PUBLISH-2** follow-on deferred by PROFILE-PUBLISH-1 (Decision AR, AC-16: "member selector / member offer surface / NO-422 contract change → PROFILE-PUBLISH-2").

**Predecessor:** Initiative 21 (E33 PROFILE-PUBLISH-1 — published `bundle_hash` in append-only store, proven by G-SMOKE; ✅ done) + Initiative 23 (E35 — `EstimateProfileSource` metadata in `EstimateView`; ✅ done).

> **Story breakdown is epic-level sketch.** Full ACs, task lists, pre-enumeration saves, test-target counts, and review routing are produced per story by `bmad-create-story` at dev-entry time. Story 36.1 now has a ready-for-dev story file; Stories 36.2–36.3 remain `backlog`. This planning artifact does NOT authorize implementation by itself — each story still requires explicit operator dev-go and story-branch execution.

### Overview

Init 24 gives members the ability to see compatible admin-published print profile offers for a model and select one to view the corresponding estimate — with E35 honest labelling (exact filament / default material / unavailable). All new code is read-only over already-published bundles and the existing estimate store; no new slicer worker path; SW-DEPLOY-1 NOT triggered. Admin API preserved unchanged.
...
| FR | Epic | Story | Notes |
|---|---|---|---|
| FR24-MEMBER-OFFER-LIST-1 | E36 | 36.1 | Member-accessible `GET /api/profiles/offers/published` — published offers only, safe DTO. |
| FR24-COMPAT-FILTER-1 | E36 | 36.1 | Optional `?material=PLA` param filters by `compatible_material_categories`. |
| FR24-MEMBER-OFFER-RESOLVE-1 | E36 | 36.2 | Estimate-by-offer resolution: `offer_id` + `stl_hash` → published `bundle_hash` → `EstimateView` with E35 source labels or `not_computed`. |
| FR24-MEMBER-OFFER-UI-1 | E36 | 36.3 | Member offer picker UI — selection updates estimate display; E35 honesty labels; unavailability UX. |

**NFR ↔ Epic / Story matrix:**

| NFR | Epic | Story | Notes |
|---|---|---|---|
| NFR24-NO-422-1 | E36 | 36.2 | No member-reachable 422 from the offer → resolve path. Realizes NFR21-NO-422-1 for the member surface. |
| NFR24-LEAKFENCE-1 | E36 | 36.1 | Member offer DTO must NOT include raw `bundle_hash`, Orca profile refs, chain body, or sidecar paths. Verified by negative DTO test. |
| NFR24-HONESTY-1 | E36 | 36.2+36.3 | E35 `EstimateProfileSource` labels flow through: exact / default / unavailable shown distinctly; fallback never appears as exact. |
| NFR24-UNAVAIL-UX-1 | E36 | 36.3 | No compatible offer or estimate unavailable → clear non-misleading UI message. |
| NFR24-AUTH-1 | E36 | 36.1+36.2 | Authenticated-member-accessible; anonymous → 401; unpublished offer → 404. |
| NFR24-AUTHGATE-1 | E36 | 36.3 | Offer picker defers to shell `AuthGate` for anonymous/unknown auth state (Init 10 authgate discipline). |
| NFR24-I18N-1 | E36 | 36.3 | `modules.member.offers.*` keys in both `en.json` and `pl.json`. |
| NFR24-VISUAL-1 | E36 | 36.3 | Playwright visual baselines for offer picker populated / empty / unavailable × 4 projects. |
| NFR24-DETERMINISM-1 | E36 | 36.1–36.3 | 3× consecutive identical pytest + vitest pass counts before merge of any story. |

### Epic List

| Epic | Name | Status | Stories |
|---|---|---|---|
| E36 | Member-Facing Published Profile Offer Surface | 🚧 backlog | 36.1 ready-for-dev; 36.2, 36.3 backlog |

#### Epic E36 — Member-Facing Published Profile Offer Surface

**Goal:** ship a member-facing offer picker that shows compatible published print profile offers, lets the member select one to see the matching estimate (with E35 honest labels), and clearly explains unavailability — all as a read-only surface over already-published bundles and existing estimates, with no change to the admin API, no Alembic migration, and no slicer worker path added.

##### Story 36.1 — Member-accessible published-offer list endpoint + safe DTO (FR24-MEMBER-OFFER-LIST-1, FR24-COMPAT-FILTER-1, NFR24-LEAKFENCE-1, NFR24-AUTH-1, NFR24-DETERMINISM-1; Decision AT)

**Sketch:** New `GET /api/profiles/offers/published` in a member-accessible router module (NOT inside `admin/` namespace; gated on `current_user` dep, not `current_admin`). Reads `profile_offer.list_offers()` → filters `publish_state == "published"` → projects to `MemberPublishedOfferView`: `{offer_id, portal_label, quality_tier, compatible_material_categories, printer_name}`. Optional `?material=PLA` param pre-filters by `compatible_material_categories`. Added to `router.py` outside `_PUBLIC_ROUTES`. Tests: 401 anonymous; 200 for authenticated member; DTO safety fence negative assertion (no `bundle_hash`, no raw Orca ref names); published-only filter; material filter. **Deploy-clean** (read-only over existing offer sidecar files → **SW-DEPLOY-1 NOT triggered**).

**Gates:** Story file exists and is ready-for-dev; implementation remains BLOCKED until explicit dev-go and story-branch execution.

##### Story 36.2 — Member estimate-by-offer resolution (FR24-MEMBER-OFFER-RESOLVE-1, NFR24-NO-422-1, NFR24-HONESTY-1, NFR24-AUTH-1, NFR24-DETERMINISM-1; Decision AT)

**Sketch:** Extend `GET /api/estimates` with an optional `offer_id` query param (OD-1 default: extend existing; confirm before dev-story). When `offer_id` is supplied: read offer sidecar → validate `publish_state == "published"` → retrieve `publish_state.bundle_hash` → read existing `EstimateStore(stl_hash, bundle_hash)` → return `EstimateView` (with E35 `estimate_profile_source`, `selected_material`, etc.) if present; if not present return `{status: "not_computed", offer_id}` (no on-demand enqueue — **G-ENQUEUE** deferred); if offer not published/not found return 404. Optional `spoolman_filament_ref` param for E35 exact-vs-default disambiguation (OD-2; default: optional, absent → material-default only). Backward-compatible: callers without `offer_id` use existing path unchanged. NFR24-NO-422-1: no branch in this path calls `resolve()` live → no 422 possible. **Deploy-clean** (backend read-only → **SW-DEPLOY-1 NOT triggered**).

**Depends on:** 36.1 on `main`. **OD-1/OD-2 operator confirmation required before dev-story.**

**Gates:** **G-DEVGO**; **G-36.1-DONE** (36.1 backend on `main`); G-UXGATE (OD-3 — UX spec required before 36.3; 36.2 backend may proceed in parallel).

##### Story 36.3 — Member UI — offer picker + estimate display (FR24-MEMBER-OFFER-UI-1, NFR24-HONESTY-1, NFR24-UNAVAIL-UX-1, NFR24-AUTHGATE-1, NFR24-I18N-1, NFR24-VISUAL-1, NFR24-DETERMINISM-1; Decision AT)

**Sketch:** New offer picker component in the model file/estimate view (exact location in `apps/web/src/modules/catalog/` or `modules/estimates/` determined at create-story via code recon). Calls 36.1's endpoint to list compatible offers for the current model's material. Renders offer selector (layout + interaction per UX spec). On selection: calls 36.2's extended `GET /api/estimates?offer_id=...&stl_hash=...` → displays `EstimateView` with E35 source label chips (exact/default/unavailable). Unavailability states: (a) no published offers → "No published profiles available"; (b) `not_computed` → "Estimate not yet available"; (c) `unavailable_no_profile` → "No filament profile for this material". AuthGate discipline: defer to shell `AuthGate` for anonymous; no component-level redirect on unknown auth state (Init 10 NFR). en+pl i18n parity. Visual baselines for populated / empty / unavailable × 4 Playwright projects. Possible route or component ripple check via [[reference_web_routetree_regen]].

**Depends on:** 36.2 on `main` + G-UXGATE (`ux-profile-publish-2-member-offer-picker` UX spec completed).

**Gates:** **G-DEVGO**; **G-UXGATE** (`bmad-ux` work item `ux-profile-publish-2-member-offer-picker` REQUIRED before FE implementation — consistent with Init 21 PROFILE-OFFER-1 UX gate precedent); **G-36.2-DONE** (36.2 on `main`).
...
| E47 | i18n + theming + visual regression + runbook/docs cutover + terminal category retirement | 🚧 backlog | 47.1 i18n, 47.2 visual specs, 47.3 runbook/docs, 47.4 *(absorbed → 47.5, 2026-07-22)*, 47.5 category API-surface + ORM + DTO + `0019` atomic cutover (absorbs 47.4) |
...
**Goal:** finish the cutover — i18n key swap, dark-mode token AC, pl-PL visual baselines for the new surfaces, the agent-runbook/docs edits that drop category references, **and the terminal category retirement relocated here from E42** (since 2026-07-22: one absorbed story — 47.5 API-surface + ORM + DTO + `0019` atomic cutover, absorbing 47.4). This is the only epic where the destructive category work lands; the live category API is a zero-code compatibility bridge until the absorbed 47.5 cutover.

**Depends on:** E44–E46 UI landed (visual specs need real surfaces). The absorbed 47.5 retires the last two live `useCategoriesTree` consumers (`AddModelForm`, `ModelHero`) *inside its own phase (a)* — the former "all consumers already gone" precondition was stale against what 45.2/45.3 actually shipped (the 2026-07-22 correct-course finding).
...
##### Story 47.4 — Category API-surface retirement + FE category-tree type/hook cleanup — **ABSORBED → 47.5** (2026-07-22) — *relocated from 42.3; FE type/hook cleanup assigned here by the 2026-07-19 E43 correct-course*

> **Absorbed 2026-07-22** by `bmad-correct-course` (SCP `sprint-change-proposal-2026-07-22-e47-4-absorbed-into-47-5.md`, operator-approved — exact GO phrase `GO E47 ATOMIC CUTOVER`, relayed by Laura from Michał, 2026-07-22). 47.4's full scope (backend API-surface retirement + FE `useCategoriesTree`/`CategoryNode`/`CategoryTree` cleanup) is folded into Story 47.5 as one atomic cutover, together with the newly-identified `AddModelForm` selector removal and `ModelHero` breadcrumb removal (no replacement UI — `TagGroupsSection` is the classification surface). Trigger: 47.4's spec-authoring run correctly self-blocked — its stated precondition ("every consumer of `GET /api/categories`/`category_id` is gone") is false: `AddModelForm.tsx:13,81` (documented 45.3↔47.5 handshake deferral) and `ModelHero.tsx:14,65` (undocumented breadcrumb dependency) both still call `useCategoriesTree()`. Blocked artifact: `_bmad-output/implementation-artifacts/spec-47-4-category-api-surface-retirement.md`. Do NOT build 47.4 as a standalone story. The original sketch is retained below for history.
...
##### Story 47.5 — Category API-surface + ORM + DTO + `0019` atomic cutover (absorbs 47.4) (FR25-SHARE-1, FR25-AGENT-1, NFR25-LEAKFENCE-1, NFR25-SCHEMA-MIGRATION-1) — *relocated from 42.5 + orphaned ORM/`0019` drop (TB-053); absorbs 47.4 per the 2026-07-22 correct-course*

**Depends on:** 45.3 (create-form category-selector removal, **same `main` HEAD deploy** — the 45.3↔47.5 handshake, unchanged). Former dependency "47.4" is dissolved: 47.4's scope is now this story's internal phases (b)-(c). **ONE commit / one deploy HEAD** (irreducibly atomic). **DESTRUCTIVE + forward-only** (destructive-go: see gate below).

**Expanded scope (2026-07-22 correct-course, SCP `sprint-change-proposal-2026-07-22-e47-4-absorbed-into-47-5.md`) — four internal phases, required order (a)→(b)→(c)→(d) within the one commit (FE stops reading before BE stops serving, before the ORM stops existing):**
- **(a) FE consumer migration (new scope):** `AddModelForm.tsx` — remove the category `<select>` (`flattenCategories`, ~:53-67, :143-144, :194-206) + the `useCategoriesTree()` call (`:81`); `POST /admin/models` payload stops sending `category_id`. `ModelHero.tsx` — remove the category-ancestry breadcrumb (ancestry-walk + render, ~:26-35, :73-77, :93-99) + its `useCategoriesTree()` call (`:65`); **no replacement breadcrumb** — `TagGroupsSection` (45.2) is the classification surface. Both components' tests drop `useCategoriesTree` mocks; visual baselines regenerated with `baseline-reviewed:` sign-off per the Baseline Acceptance Gate.
- **(b) FE hook/types retirement (ex-47.4 FE half):** delete `useCategoriesTree.ts` + its test; delete FE `CategoryNode`/`CategoryTree` (`api-types.ts:55-59,61-63`). FE `CategorySummary` falls in (d), not here.
- **(c) BE API-surface retirement (ex-47.4 backend half, Code Map carried verbatim from `spec-47-4-category-api-surface-retirement.md`):** delete `GET /api/categories` (`sot/router.py:48-64`), admin category CRUD (`admin_router.py:765-853`), `list_categories_tree` (`service.py:63-124`), `create/update/delete_category` + `_would_cycle` (`admin_service.py:~1303-1479`), route-only `CategoryNode`/`CategoryTree` (`schemas.py:26-32`) + `CategoryCreate`/`CategoryPatch` (`admin_schemas.py:291-329`); delete/update the dedicated tests per the spec's Code Map. No `_PUBLIC_ROUTES` edit (TB-055).
- **(d) BE DTO + ORM + migration (original 47.5 scope, unchanged):** the sketch below.
...
> **Destructive gate (MANDATORY before any 47.5 dev-go / deploy).** Operator destructive-go must acknowledge: (1) `0019` is forward-only, `downgrade()` raises — irreversible by Alembic; (2) category rows + per-model assignment permanently lost by design (accepted in the 2026-07-17 SCP — models start untagged); (3) rollback = verified pre-`0019` `.190` DB backup restore + `git revert` of the **whole absorbed cutover commit** (a)-(d) + redeploy (a partial revert would leave the app inconsistent); (4) a **fresh** verified pre-`0019` DB backup taken on `.190` under the deploy lock (`flock /tmp/3d-portal-deploy.lock`) immediately before merge/deploy, with **demonstrated restore-readiness** (scratch-restore/verification of that file, command + output logged to `.hermes/run-logs/`) — a backup that exists but was never opened/verified is not restore-readiness; (5) the absorbed 47.5 cutover is the only substantive change in its deploy; (6) single Alembic head after `0019`.
>
> **Destructive-go RECORDED 2026-07-22:** operator (Michał/Ezop) approved via the exact phrase `GO E47 ATOMIC CUTOVER` (controller session, relayed by Laura, 2026-07-22), against an interpretation explicitly presented beforehand that named: absorption of 47.4 into 47.5, `AddModelForm` selector removal, `ModelHero` breadcrumb removal with no replacement, and **destructive implementation + deployment of forward-only `0019_drop_category` accepting deletion of 43 category rows and 130 model-category assignments from the live DB**. Restore-readiness was demonstrated pre-GO: backup `portal-pre-e47-5-20260722T172024Z.db` (sha256 `0dfd305e…d21652`, integrity ok, FK 0, alembic `0018_facet_tags`, 43/130 counts, independent scratch restore ok; evidence `.hermes/run-logs/e47-precutover-backup-20260722T172024Z.log`). That checkpoint does **not** replace gate item (4): a second fresh verified backup under the deploy lock is still mandatory immediately before deploy. (Do NOT create the implementation-ready story in the correct-course pass — `bmad-create-story` runs on the merged scope next.)
...
**Added 2026-08-01 by `bmad-correct-course`** — SCP `sprint-change-proposal-2026-08-01-e53-android-lightbox-fit-defect.md`. **This is a bugfix against shipped behaviour, not new scope**, and it is the reason E53 has open work as well as an open gate.

**Sketch:** on a **physical** Pixel 9 Pro running **Brave 1.92.144 / Chromium 150.0.7871.186 on Android 17**, the shipped `/catalog/$modelId` lightbox **does not fit the photo inside the visible frame at reset / minimum scale**: the left `1vw` inset is honoured, there is no matching right gutter, the photo is clipped by the right screen edge, and — the load-bearing observation — **the bottom-right zoom toolbar is clipped too**, so this is the whole viewer box overflowing to the right and not merely image sizing. Meanwhile `image-viewer-containment.spec.ts` asserts containment at scale 1 across the geometry matrix in both orientations and reports green. **The automated evidence and the physical evidence disagree, and reconciling them is half this story.** Work the sequence in order: (1) **reproduce deterministically** — identify and write down the actual viewport/browser condition, never "sometimes on a phone", and record the exact deployed build; (2) **add a regression that FAILS FIRST** and state explicitly **why the existing suite could not fail** (candidate: emulated `mobile-*` projects give layout viewport == visual viewport == device width with overlay scrollbars, so the failure mode is unrepresentable there); (3) **minimal root-cause fix** — not a clamp that hides the geometry; (4) **preserve zoom/pan inspectability above 1.0** — no fix that fits the image by weakening the capability the epic exists to deliver; (5) **deploy**, recording the version; (6) **request a fresh physical retest**. **Candidate surfaces, hypotheses only — the story owns the diagnosis and may reject all of them:** the `w-[98vw]` / `left-[1vw]` anchoring if `vw` resolves against the layout viewport rather than the visual viewport as the in-file comment asserts; the `max-w-[calc(100%-2vw)]` counterweight, which its own comment concedes headless CI cannot exercise; Brave/Android 17 chrome and edge-to-edge viewport behaviour; `object-contain` + `max-w-full` fitting an already-over-wide box (insufficient alone — it would not explain the clipped toolbar). **Boundaries:** every Story 48.1 / Decision BA invariant stays binding (single reference box, `dvh` not `vh`, never `user-scalable=no`); `apps/web/src/ui/dialog.tsx` remains **Ask First**; visual-baseline churn must be justified per PNG, never blanket-refreshed; and the other open E53 residuals (the `/share` never-mounted-`<img>` and mounted-and-hung `/catalog` entries in `deferred-work.md`, the `ui/dialog.tsx` focus trap routed to 54.2, the double-tap chrome flash awaiting a 53.2 AC-3 amendment) are **explicitly NOT absorbed here** — they stay owed to a separate follow-up story. **Gate honesty:** the triggering evidence is **Brave, not Chrome for Android**, and it is **defect** evidence rather than gesture-acceptance evidence, so **`G26-LIB` is not advanced by one line of this story**; `G26-LIB` and `epic-53` stay OPEN until a **deployed** repair earns a fresh physical retest, and Story 53.4 closing closes neither. `VERIFY-AT-CREATE-STORY`: the shipped geometry as it actually stands on `main`, and what the containment suite does and does not measure.
