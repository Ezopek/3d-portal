...
**Status:** 🚧 planning (started 2026-06-07). Source SCP: `sprint-change-proposal-2026-06-07-spoolman-filament-profile-estimates.md` (status `approved` — operator delegated implementation of the first slice 35.1 in this run; later stories gated on per-story `bmad-create-story` + dev-go). Predecessor: Init 20 (STL Slicer Estimates, Epic E32 — the resolver / `(stl_hash, bundle_hash)` cache / override-layer this builds on) + Init 19 (Spoolman Read-Only Inventory, Epic E31 — the cached snapshot + `spoolman_filament_ref()` reused, NO second live read). Single epic **E35**. Architecture: `architecture.md` § Initiative 23 (Decision AS — portal-owned profile-selection policy + `EstimateProfileSource` classification; to be appended). Distinct from Init 21 (Epic E33 — admin-managed Orca **process** profiles + offer/chain): this initiative owns the **filament**-profile selection policy keyed by Spoolman generic material. Keep branch/diff boundaries clean from `fix/slicer-repair-fallback-estimates` (SCP § "do not bundle").

> **Story breakdown is epic-level sketch.** Full acceptance criteria, task lists, pre-enumeration saves, test-target counts, and Codex routing are produced per story by `bmad-create-story` at dev-entry time. Authoring these planning artifacts did NOT authorize dev-story execution beyond the operator-delegated **35.1** slice — later stories (35.2–35.6) stay gated on per-story `bmad-create-story` + an explicit dev-go.

### Overview

Init 23 makes print estimates pick the Orca **filament** profile from a portal-owned **profile-selection policy** rather than the single material-class default the Init 20 resolver hard-wires today. Spoolman remains the source of truth for filament inventory and the generic material type (`PLA`, `PETG`, `PCTG`, `TPU`, `ABS`, …); the portal owns the mapping from that material — and optionally from a specific Spoolman filament — to a concrete Orca `FilamentProfile`. Resolution precedence is **exact filament override > material-type default > unavailable**. A material-default estimate is allowed but must be labelled **default/fallback**, never exact; a missing profile yields a classified **`unavailable_no_profile`** absence that does NOT block an order/request. The bounded **default matrix** (catalog STL × active offer/process × compatible configured material-default profile) is precomputed so normal generic-material estimates are ready immediately; concrete per-filament overrides are an explicit operator-managed exception path that is NOT auto-precomputed. The cache identity stays `(stl_hash, bundle_hash)` — `bundle_hash` already folds the selected filament profile/override layer (Init 20 Decision AH), so two Spoolman filaments that resolve to the same Orca filament profile share a bundle, while an exact-override filament resolves to a distinct bundle.
...
| FR | Epic | Story | Notes |
|---|---|---|---|
| FR23-POLICY-1 | E35 | 35.1 | Portal-owned profile-selection policy model + file-backed store (material defaults + per-filament overrides); material-key normalize (trim + uppercase); atomic write + mtime-cached load. Anchors Decision AS. |
| FR23-PRECEDENCE-1 | E35 | 35.1 | Pure precedence resolver: exact override > material default > unavailable; disabled entries fall through; result carries `EstimateProfileSource` + selected ref/material. |
| FR23-RESOLVER-1 | E35 | 35.2 | Resolve the Orca filament profile by policy before bundle materialization; preserve byte-identical default path when no filament selected; two generic PLA colors share `bundle_hash`, PLA Matt exact override differs. |
| FR23-SNAPSHOT-MAP-1 | E35 | 35.2/35.3 | Map the Init 19 cached Spoolman snapshot → generic material + stable `spoolman_filament_ref`; soft-fail when snapshot unavailable; no second live read; counts-only logs. |
| FR23-ESTIMATE-API-1 | E35 | 35.3 | Estimate ingest/read APIs accept/derive the selected filament; return `exact/default/unavailable` source + selected material/ref/name + Orca profile name; preserve no-selected-filament catalog behavior + `fresh/stale/queued/failed` semantics. |
| FR23-ADMIN-1 | E35 | 35.4 | Admin policy management surface (list Spoolman materials + known Orca profiles; get/update material defaults + per-filament overrides), validated server-side, no restart. |
| FR23-UI-LABEL-1 | E35 | 35.5 | User-facing estimate UI labels exact vs default-fallback vs unavailable distinctly; Orca internals stay admin/debug-scoped. |
| FR23-BACKFILL-1 | E35 | 35.6 | Bounded default-matrix enqueue/backfill hooks + dry-run counts by `(offer/process, material, profile_source)`; exact overrides excluded unless explicitly operator-requested; `(stl_hash, bundle_hash)` dedupe is the safety valve. |

**NFR ↔ Epic / Story matrix:**

| NFR | Epic | Story | Notes |
|---|---|---|---|
| NFR23-CACHE-INVARIANT-1 | E35 | 35.1/35.2 | Preserve Init 20 `(stl_hash, bundle_hash)` dedupe/cache semantics; policy selection changes the resolved filament JSON (hence `bundle_hash`) but never the hashing contract. |
| NFR23-HONESTY-1 | E35 | 35.3/35.5 | Default/fallback estimates labelled distinctly from exact; `unavailable_no_profile` shows no time/grams/cost but keeps request/order path. The primary product risk. |
| NFR23-NO-BLOCK-1 | E35 | 35.3/35.5 | Missing material profile ⇒ estimate unavailable, never an order/request block. |
| NFR23-STABLE-KEY-1 | E35 | 35.1+ | Key exact overrides by `spoolman_filament_ref()` (vendor∥material∥name), NEVER the churning Spoolman integer id. |
| NFR23-OBS-1 | E35 | 35.2/35.3 | Logs/breadcrumbs carry profile-source labels + counts/reason categories only — never full filament bodies (mirrors Init 20 NFR20-OBS-1). |
| NFR23-DETERMINISM-1 | E35 | 35.1+ | Pure policy resolution + 3× consecutive identical pytest counts before merge for code-bearing stories. |
| NFR23-QUEUE-BOUND-1 | E35 | 35.6 | No concrete-filament N×M×K precompute; lazy + bounded default matrix only; respect slicer-worker concurrency + compose CPU limits. |

### Epic List

| Epic | Name | Status | Stories |
|---|---|---|---|
| E35 | Spoolman Filament Profile Estimate Policy | 🚧 in-progress | 35.1 (policy store/precedence — first), 35.2 (resolver/bundle integration), 35.3 (estimate API + DTO metadata), 35.4 (admin surface), 35.5 (UI source labels), 35.6 (bounded backfill) |

#### Epic E35 — Spoolman Filament Profile Estimate Policy

**Goal:** estimates select the Orca filament profile from Spoolman-backed generic-material defaults plus optional concrete-filament overrides, so new normal filaments estimate automatically while unusual filaments can be pinned by an admin — with honest exact/default/unavailable labelling and the Init 20 cache invariants preserved. Sliced read/foundation-first (mirrors the proven 33.1 / 34.1 read-first cadence): the pure policy store + precedence resolver land first (35.1, standalone + fully unit-testable, deploy-clean), then the resolver/bundle integration (35.2), then the API/DTO surface (35.3), admin (35.4), UI (35.5), and backfill ergonomics (35.6).
...
**Realizes:** the policy foundation Tasks 1 of the SCP — a portal-owned profile-selection policy (material defaults + per-filament overrides) + the `EstimateProfileSource` classification + a pure precedence resolver. **Pure / standalone / deploy-clean** (no resolver, API, or worker coupling yet; no Alembic; no on-disk worker change → SW-DEPLOY-1 NOT triggered).

**Sketch:** new `apps/api/app/modules/slicer/profile_policy.py` carrying: the `EstimateProfileSource` StrEnum (`exact_filament_mapping` / `default_material_profile` / `unavailable_no_profile`); `normalize_material()` (trim + uppercase; empty/blank → `None` = unconfigured, never an invented variant); Pydantic v2 policy models (`MaterialDefault`, `FilamentOverride`, `ProfilePolicy`) with `extra="forbid"` and material-default keys normalized at the boundary; a `ProfileSelection` result (source + `orca_filament_profile_ref` + `selected_material` + `selected_spoolman_filament_ref`); a pure `ProfilePolicy.resolve_selection(material, spoolman_filament_ref)` implementing exact-override > material-default > unavailable with disabled entries falling through; a filesystem-backed `ProfilePolicyStore` (atomic publish + flock + mtime-cached load mirroring `attribution_store` / `estimate_store`) reading/writing a single `profile_policy.json`; an `unknown_profile_refs(policy, known_refs)` validation seam (pure — no hard-coded concrete Orca refs); a settings slot (`slicer_profile_policy_dir`) + a settings-wired `load_profile_policy()` convenience.

**Depends on:** the Init 20 `spoolman_filament_ref()` + `MaterialClass` (shipped). **No UX-design blocker** (no UI in this slice).

**Acceptance boundaries (sketched; finalized by bmad-create-story):**
- Material normalization: `" pla "` → `"PLA"`; `""`/whitespace/`None` → `None` (unconfigured, not coerced).
- Precedence: exact override wins over material default; material default used when no override; no override + no default ⇒ `unavailable_no_profile` with `orca_filament_profile_ref=None`.
- Disabled fall-through: a `enabled=false` exact override falls through to the material default; a disabled material default ⇒ unavailable.
- Selection carries the chosen `orca_filament_profile_ref` (None when unavailable), the normalized `selected_material`, and the `selected_spoolman_filament_ref` only on an exact mapping.
- Store: save→load roundtrip is identity; load of a missing file returns an empty policy; save is atomic (no partial file); load is mtime-cached and re-reads after a save.
- Stable key: overrides are keyed by `spoolman_filament_ref()`, never the integer id.
- Validation seam: `unknown_profile_refs` returns refs absent from a supplied `known_refs` set, with NO concrete Orca ref hard-coded in the model/tests.

**Test targets (sketched):** pytest `tests/test_slicer_profile_policy.py` — normalization table; precedence (exact/default/unavailable); disabled fall-through (override + default); selection field carry; store roundtrip / missing-file-empty / atomic-overwrite / mtime-reload; `unknown_profile_refs` seam. Determinism: 3× consecutive identical counts. (No vitest / no visual — backend-only slice.)

**Out of scope (deferred to later E35 stories):** resolver/bundle wiring (35.2); Spoolman snapshot → material/ref map (35.2/35.3); estimate ingest/read API + DTO metadata (35.3); admin surface (35.4); UI labels (35.5); bounded backfill (35.6).
...
**Sketch (epic-level):** extend the resolver input/result with the selected Spoolman filament + the chosen `EstimateProfileSource`; map the Init 19 cached snapshot → generic material + stable ref (reuse `SpoolsService.get_summary()` + `spoolman_filament_ref()`, no second live read, soft-fail to material-default/unavailable); select the Orca filament profile by policy before bundle materialization; keep the no-selected-filament default path byte-identical; classified absence (no enqueue) when unavailable. **Acceptance headline:** two generic PLA colors that resolve to the same default profile share `bundle_hash`; a PLA Matt exact override resolves to a different `bundle_hash`. Finalized by `bmad-create-story`.

##### Story 35.3 — Estimate ingest/read API + DTO source metadata (FR23-ESTIMATE-API-1, NFR23-HONESTY-1, NFR23-NO-BLOCK-1, NFR23-OBS-1)

**Sketch (epic-level):** estimate read/recompute/enqueue paths accept/derive the selected filament; responses carry `estimate_profile_source` + `selected_material` + `selected_spoolman_filament_ref` + `selected_filament_name` + `orca_filament_profile_name`; preserve current catalog/no-selected-filament behavior + `fresh/stale/queued/failed` semantics; `unavailable_no_profile` returns absence (no enqueue) without blocking order/request; profile-source labels in logs + breadcrumbs. Finalized by `bmad-create-story`.

##### Story 35.4 — Admin policy management surface (FR23-ADMIN-1)

**Sketch (epic-level):** admin endpoints to list Spoolman material types (from snapshot) + known Orca filament profiles, and get/update material defaults + per-filament overrides; server-side validation (profile exists via the 35.1 seam, material normalized, ref stable); no restart (policy store read-on-demand / mtime-cached). FE admin tables for material defaults (configured/unconfigured) + per-filament overrides. Requires a `bmad-ux` checkpoint before/with FE work. Finalized by `bmad-create-story`.

##### Story 35.5 — User-facing estimate UI source labels (FR23-UI-LABEL-1, NFR23-HONESTY-1, NFR23-NO-BLOCK-1)

**Sketch (epic-level):** badges/tooltips for profile source — exact (confident display), default (subtle "domyślny profil PLA" badge), unavailable (no time/grams/cost, request/order path intact); Orca internals stay admin/debug-scoped. en+pl i18n parity + mandatory visual regression. Finalized by `bmad-create-story`.

##### Story 35.6 — Bounded default-matrix backfill + enqueue guardrails (FR23-BACKFILL-1, NFR23-QUEUE-BOUND-1)

**Sketch (epic-level):** backfill the bounded default matrix (catalog STL × active offer/process × compatible configured material-default profile); enqueue hooks on STL ingest + offer publish + material-default change (re-enqueue/mark-stale); exclude concrete overrides unless explicitly operator-requested; dry-run reports counts by `(offer/process, material, profile_source)`; `(stl_hash, bundle_hash)` dedupe remains the safety valve; respect slicer-worker concurrency + compose CPU limits. Finalized by `bmad-create-story`.

**Gates:** **G-DEVGO** (later stories 35.2–35.6 gated on per-story `bmad-create-story` + operator dev-go; 35.1 delegated in this run); **G-UXGATE** (a `bmad-ux` checkpoint required before/with 35.4 + 35.5 FE work); **G-ARCH** (append architecture.md § Initiative 23 Decision AS before 35.2 resolver integration); **G-BACKFILL-OPT-IN** (concrete per-filament override backfill is operator-opt-in only, never part of the default matrix).
