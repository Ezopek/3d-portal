...
**Default-deny auth posture (Init 6 Decision M — load-bearing).** The `/api/*` default-deny enumeration test (pytest `_PUBLIC_ROUTES` allowlist) will fail for any new public route not listed. The new public catalog reads (`GET /api/tag-groups`, and `GET /api/tags`/`GET /api/models` if their route registration changes) must be added to `_PUBLIC_ROUTES`; the removed `GET /api/categories` entry must be deleted from the allowlist in the same change. Admin group-governance routes stay `current_admin`-gated (not public), consistent with the existing category-CRUD posture they replace.
