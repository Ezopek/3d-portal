...
**Last updated:** 2026-07-17 (Initiative 25 Facet Tag Taxonomy + Category Retirement — MAJOR catalog rebuild via sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md, status `approved` 2026-07-17 — Decisions AU (data model: `TagGroup` + `Tag.group_id`, `Category` + `Model.category_id` removed) + AV (Alembic `0018_facet_tags_drop_category`, forward-only) + AW (API contract: facet filtering, category endpoints removed, share DTO) appended after Initiative 24 H2. Schema-migrating + full catalog contract change; implementation BLOCKED until bmad-create-story + dev-go. **Update 2026-07-19 (bmad-correct-course, SCP `sprint-change-proposal-2026-07-19-e42-deferred-coupled-cutover.md`):** the single `0018_facet_tags_drop_category` shipped **split** into additive `0018_facet_tags` (E41) + deferred terminal destructive `0019_drop_category` (Story 47.5, forward-only, destructive-go DEFERRED); the category-endpoint/DTO/ORM removals relocated from E42 to E47 (47.4/47.5) with the live category API as a zero-code compatibility bridge; TB-055 auth-posture prose corrected (SoT reads authenticated, outside `_PUBLIC_ROUTES`). See Decisions AV/AW Updates below. Prior: 2026-06-13 Initiative 24 Decision AT.)
...
| 25 | Facet Tag Taxonomy + Category Retirement (Catalog Rebuild) | 🚧 planning | — | 3 architectural decisions (AU — data model: add `TagGroup` (`tag_group`); add `Tag.group_id` nullable FK `SET NULL` + `Tag.group_position`; **remove** `Category` entity and `Model.category_id` (its FK + `ix_model_category_id` index); many-to-many `model_tag` retained unchanged (`tag_id` RESTRICT). AV — migration plan **split in execution**: shipped, reversible, additive-only `0018_facet_tags` (`down_revision = "0017_model_note_bilingual"`) creates `tag_group` and adds `tag.group_id`/`group_position`; terminal Story 47.5 owns forward-only `0019_drop_category` (`down_revision = "0018_facet_tags"`) in the same atomic commit as ORM/DTO category removal, with verified pre-deploy DB backup and destructive-go deferred to 47.5 dev-go. AW — API contract: `GET /api/models` drops `category_ids`, adds `tag_ids`/`tag_match`(`all`|`any`, default `all`)/`untagged` with AND-between-groups / OR-within-group semantics; `GET /api/tags` gains `group`+`?with_counts`; new `GET /api/tag-groups`; **remove** `GET /api/categories` + admin category CRUD + `Category*` schemas; `ModelCreate`/`ModelPatch` drop `category_id`; `ShareModelView` drops `category` (live projection, no migration); **reuse** existing `POST /tags/merge`; new admin `POST/PATCH/DELETE /api/admin/tag-groups` + move-to-group). Full catalog UI + admin governance rework; epics 41–47. `slicer/*` "category" is a false positive (out of scope). Source SCP: `sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md`. See section "Initiative 25" below. |
...
**Status:** 🚧 planning (started 2026-07-17). Source SCP: `sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md` (status **approved** by operator 2026-07-17, routed Path A: PM/Architect → Sprint Planning). Design source of truth (visual): `docs/design/HANDOFF-tagi-fasetowe.md` (7 mockups + edge states, PL/EN + light/dark). Epics 41–47 (see `epics.md` § Initiative 25). **Change class: MAJOR** — retires a shipped core entity (`Category`), requires a schema migration (`0018`), and rebuilds the full catalog browse/filter/detail/admin surface.

### Overview

The catalog was built on a **single, hard, mandatory category per model** (`Model.category_id` NOT NULL → `category.id`, organised as a self-referential tree). Operating it exposed the classic single-tree taxonomy failure: one model legitimately belongs to several axes at once (type, room, system, material, creator), and a single tree forces a lossy "pick one" plus deep drill-downs. Owner decision (2026-07-16): replace the hard single category with **many tags grouped into facets**. The many-to-many `model_tag` join already exists and is leaned on unchanged; the net-new entity is a **facet group** (`TagGroup`) that a tag optionally belongs to.

Three load-bearing framing choices set by the SCP and carried into the decisions below:
...
- **Tag creation is admin-only.** Users pick from a curated set; the create path is admin-gated. Facet **groups** are a real admin-managed entity, not a string convention.
- **Filter combination is a user-visible AND/OR toggle** (default AND-between-groups, OR-within-group), with `tag_match` as the explicit override.

`slicer/*` "category" hits (`reason_category`, warning `code`, incompatibility strings) are a **false positive** — unrelated to the `Category` entity (no import) — and are explicitly OUT OF SCOPE (SCP § 2.4.2). This prevents a large, wrong scope expansion.

### Decision AU — Data model: facet groups replace the category tree

**Accepted (approved)** 2026-07-17 for Initiative 25, per SCP § 4.2. Target file `apps/api/app/core/db/models/_entities.py`.

- **Add `TagGroup` (`__tablename__ = "tag_group"`):** `id` PK (uuid), `slug` (unique + index), `name_en`, `name_pl: str | None`, `position: int` (default 0, group ordering), `created_at`, `updated_at`. Groups are the admin-managed facet axis (Type, Room, System, Material, Creator, …).
- **Amend `Tag`:** add `group_id: uuid.UUID | None` FK → `tag_group.id`, **`ondelete="SET NULL"`**, nullable; add `group_position: int = 0` (per-tag order within its group). Retain existing `slug` unique + index, `name_en`, `name_pl`.
- **Remove `class Category` entirely** — the self-referential tree (`parent_id` RESTRICT self-FK, `uq_category_parent_slug`, `uq_category_root_slug`, `ix_category_parent`, current `_entities.py:29-58`).
- **Amend `Model`:** remove the `category_id` field together with its `category.id` FK and `ix_model_category_id` index (current `_entities.py:85-87`).

**Why `SET NULL` on `tag.group_id` (not RESTRICT).** A tag must survive deletion of its group — deleting a facet group must **not** cascade-delete or block on the curated tags inside it; the tags simply become **groupless** and fall under the "Untagged"/groupless pseudo-facet (FR25-TAX-1). This is deliberately the opposite posture from `model_tag.tag_id` (which stays **RESTRICT** — a tag still cannot be deleted while models reference it; delete requires merge-or-empty first via the existing `POST /tags/merge`). Nullability of `group_id` is load-bearing, not incidental: the groupless state is a first-class rendering case, not an error.

**Invariants preserved:**
- `model_tag` M2M join + `tag_id` `ondelete="RESTRICT"` — **unchanged** (`_entities.py:61-69,129-139`). The facet story is built entirely on the existing join, not a new association.
- `Tag.slug` uniqueness — unchanged.
- No other entity touched; `Category` has no dependents beyond `Model.category_id` and the live share projection (Decision AW).

### Decision AV — Alembic `0018_facet_tags_drop_category` (forward-only)

> **⚠️ Update 2026-07-19 (bmad-correct-course; SCP `sprint-change-proposal-2026-07-19-e42-deferred-coupled-cutover.md`) — implementation split into two migrations, superseding the single-migration text below.** The planned single `0018_facet_tags_drop_category` was **split during execution** (E41 retro decision) into: **(1)** shipped `0018_facet_tags` — **additive-only, reversible** (`create tag_group`, add `tag.group_id`/`group_position`; `upgrade()` has zero `model`/`category_id`/`category` references; `category` + `model.category_id` deliberately **survive** `upgrade(head)`, executably proven by `test_migration_0018`); and **(2)** the deferred **terminal** `0019_drop_category` — **destructive + forward-only** (`down_revision = "0018_facet_tags"`, `downgrade()` raises `NotImplementedError`), owned by **Story 47.5** (formerly the E42 tail), which must land the `batch_alter_table("model")` `drop_index("ix_model_category_id")` + `drop_column("category_id")` (no `drop_constraint` — the `category.id` FK is an unnamed inline, per the name introspection below) + `op.drop_table("category")` in the **same commit** as the ORM removal of `class Category` + `Model.category_id`. The name-introspection facts below are correct and carry to `0019`. Destructive-go for `0019` is **DEFERRED to 47.5 dev-go** (verified pre-`0019` `.190` DB backup required). Read the paragraph below as describing the *combined* intent; the *shipped* split is authoritative.

**Accepted (approved)** 2026-07-17 for Initiative 25, per SCP § 4.2 and the code-verified name introspection below. New revision `0018_facet_tags_drop_category`; **`down_revision = "0017_model_note_bilingual"`** (full-slug revision id per repo convention — `0017` alone is not a valid revision string, `migrations/versions/0017_model_note_bilingual.py:28`). **Forward-only** per `NFR10-SCHEMA-MIGRATION-1` (repo migration precedent): `downgrade()` raises `NotImplementedError` — dropped categories and per-model assignment are unrecoverable by design.

**Upgrade order (each step depends on the prior):**
1. **`op.create_table("tag_group", …)`** — columns per Decision AU; then `op.create_index("uq_tag_group_slug", "tag_group", ["slug"], unique=True)`.
2. **`with op.batch_alter_table("tag") as b:`** — `b.add_column(group_id, nullable=True)`; `b.add_column(group_position, nullable=False, server_default="0")`; `b.create_foreign_key("fk_tag_group_id", "tag_group", ["group_id"], ["id"], ondelete="SET NULL")`. (This FK **is** named — we create it here; precedent for a named batch FK: `0005:113` `fk_model_note_author_id`.)
3. **`with op.batch_alter_table("model") as b:`** — `b.drop_index("ix_model_category_id")` then `b.drop_column("category_id")`. **No `drop_constraint` call.**
4. **`op.drop_table("category")`** — wholesale; the self-FK (`parent_id` RESTRICT), `uq_category_parent_slug`, `uq_category_root_slug`, and `ix_category_parent` are dropped with the table. No leaf-by-leaf delete — we are dropping the table, not clearing rows.

**Name introspection (RESOLVED — do not re-guess).** The SCP flagged the `model.category_id` FK/index names as "confirm, don't guess." Resolved against the creating migration `migrations/versions/0004_entity_tables.py`:
- **Index is named `ix_model_category_id`** (`0004:91`, `op.create_index("ix_model_category_id", "model", ["category_id"])`) — drop it explicitly inside the batch block (step 3).
- **The `category.id` FK is an *unnamed inline* `sa.ForeignKey`** — declared inside `create_table` as `sa.Column("category_id", …, sa.ForeignKey("category.id", ondelete="RESTRICT"), nullable=False)` (`0004:73-78`), with **no `name=`**. There is therefore **no `fk_model_category_id` constraint to drop** — the SCP § 4.2 sketch line calling `drop_constraint("fk_model_category_id")` is **incorrect and must be omitted**. On SQLite the `batch_alter_table` copy-and-move strategy recreates `model` without the `category_id` column, and the inline FK vanishes with it. Two in-repo precedents confirm this exact pattern: `0010_drop_model_legacy_id.py:36-38` drops a `model` column via `drop_index` + `drop_column` with no `drop_constraint`; `0005_user_uuid_audit_log.py:125-126` (downgrade) drops the FK-bearing `model_note.author_id` column with a bare `drop_column`, relying on the batch rebuild to remove the FK.

**Seed policy.** Migration `0018` stays **minimal and structural only** — it does **not** seed the starter taxonomy. Seeding the facet `tag_group` + `tag` rows is a **separate story (41.3)**, keeping the destructive schema migration free of content decisions (SCP § 5 open item — recommendation adopted).

**Invariants preserved:** forward-only posture (old revision `0017` retained for emergency revert); `model_tag` and `tag` rows untouched by the migration except for the two additive `tag` columns; SQLite `batch_alter_table` used for every `model`/`tag` column mutation (mandatory on SQLite, repo precedent `0010`).

### Decision AW — API contract: facet filtering, category-endpoint retirement, share projection

**Accepted (approved)** 2026-07-17 for Initiative 25, per SCP § 4.2. Targets `apps/api/app/modules/sot` (`router.py`, `admin_router.py`, schemas), `apps/api/app/modules/share`, and `apps/web/src/lib/api-types.ts`.

**Read surface (`sot/router.py`, current anchors `:46-134`):**
- **`GET /api/models`** — **drop `category_ids`**; add `tag_ids: list[str]`, `tag_match: "all" | "any"` (default `"all"`), `untagged: bool`. Semantics: **AND between groups, OR within a group**, with `tag_match` as the user override (mockup 04). Note this changes today's behaviour where `category_ids` was OR and `tag_ids` was AND — the group-aware AND/OR replaces both. `untagged=true` returns only models with zero tags (triage of the post-cutover reset).
- **`GET /api/tags`** — return each tag's `group`/`group_id` (+ `group_position`); add `?with_counts=true` for per-tag model counts.
- **New `GET /api/tag-groups`** — groups + their tags + per-tag counts, backing the frontend `useTagGroups()` facet sidebar. Single round-trip; the FE does client-side substring search over already-fetched groups (no new search endpoint).

**Removals:**
- **`GET /api/categories`** (`sot/router.py`) and admin **`POST/PATCH/DELETE /api/categories`** (`admin_router.py`, current `:770+`) + `create/update/delete_category` service fns + all `Category*` request/response schemas.
- **`ModelCreate` / `ModelPatch`** — drop `category_id`. Model create/patch no longer requires a category (`admin_router.py:148/167` 400-on-category-not-found path removed); tagging is **optional at create** (a model may be created untagged).

**Admin governance (new + reuse):**
- **New** `POST/PATCH/DELETE /api/admin/tag-groups` (group create / rename+reorder / delete) and **move-to-group** (`PATCH` a tag's `group_id`).
- **Reuse `POST /tags/merge`** — it already exists (`admin_router.py:693`, with rename/patch/delete siblings). The HANDOFF framed merge as new work; diligence (SCP § 2.4.3) confirmed it is **reuse, not build**. New admin work is group governance only.

**Share projection (`share/models.py`, `share/router.py` — diligence § 2.4.1):**
- **`ShareModelView` drops `category: str`** (currently `share/models.py:40`, resolved live from `model.category_id` at `share/router.py:144` and emitted at `:228` as `category=category.slug`). This is a **live projection, not a stored column — no data migration needed**; only the anonymous share DTO + share detail UI change. `ShareModelView.tags: list[str]` already flows to anonymous visitors, so the facet story extends cleanly onto the existing anonymous contract.

**Frontend types (`apps/web/src/lib/api-types.ts`, current `:43-63,166,183`).** *(Corrected 2026-07-19 — SCP `sprint-change-proposal-2026-07-19-e43-fe-data-additive-correction.md`, operator-approved; supersedes the pre-relocation bullets, which named a nonexistent embedded `TagRead.group` and folded the FE category removals into E43.)* The E43 frontend type work is **additive**; the category removals are relocated to the terminal cutover (47.4 tree types + hook / 47.5 model DTO + field):
- `TagRead`: add **`group_id` + `group_position` only** — the exact shipped 42.2 wire (`sot/schemas.py:35-47`, D-SHAPE-1). There is **no** embedded `group` label/slug on `TagRead`; the human group label is delivered authoritatively by `GET /api/tag-groups`. (The earlier "add `group`" bullet named a field the backend never ships.)
- Add the **shipped group-read/response types** so the FE mirrors the 42.2 contract: `TagListItem` (`TagRead` + optional `model_count?: number`, key dropped when absent), `TagReadWithCount` (`TagRead` + required `model_count: number`), `TagGroupRead` (`{ id, slug, name_en, name_pl, position, tags: TagReadWithCount[] }`), `TagGroupsResponse` (`{ groups: TagGroupRead[]; groupless: TagReadWithCount[] }`), and `TagGroupSummary` (flat admin write-response, no `tags[]`, mirrors `CategorySummary`).
- **`ModelSummary.category_id` / `ModelDetail.category: CategorySummary` removal → relocated to 47.5** (the atomic model-DTO + ORM + `0019` cutover — dropping the FE field must be same-`main`-HEAD with the backend drop; kept alive as a compatibility bridge until then).
- **`CategoryNode` / `CategoryTree` (FE types) + `useCategoriesTree` (hook) deletion → relocated to 47.4** (after all E44/E45 consumers migrate; the symmetric FE half of removing the route-only `CategoryNode`/`CategoryTree` backend schemas). **`CategorySummary` (FE type)** stays until 47.5 (embedded in the retained `ModelDetail`).

**Default-deny auth posture (Init 6 Decision M — load-bearing).** *(Corrected 2026-07-19 — TB-055; superseding the earlier draft of this paragraph, which wrongly labelled the SoT catalog reads "public" and prescribed `_PUBLIC_ROUTES` edits.)* The Init 25 SoT catalog reads (`GET /api/models`, `GET /api/tags`, new `GET /api/tag-groups`) are **authenticated default-deny** — gated on `current_user` (any role), **outside `_PUBLIC_ROUTES`** — exactly like the category reads they replace. `_PUBLIC_ROUTES` (`apps/api/app/main.py:50-61`) is `/api/health`, `/api/auth/*`, `/api/share/*` **only**; `GET /api/categories`/`/api/tags`/`/api/models` were **never** in it, so removing category routes needs **no allowlist edit**, and adding the new reads needs none either (Story 42.1 kept `GET /api/models` on `current_user`; Story 42.2 did the same for `GET /api/tags` + `GET /api/tag-groups`, with anonymous-401 / role-200 boundary tests — the authoritative recent precedent). `test_sot_auth_boundary.py` asserts anonymous → 401 for the SoT reads; putting any of them in `_PUBLIC_ROUTES` would flip anonymous to 200 and break that gate. The `test_route_enforcement_gate.py` enumeration test keys off the live route table, so it self-heals when category routes are removed. The (correct) general rule stands: a *genuinely new anonymous* route would need a `_PUBLIC_ROUTES` entry + the enumeration test — but **none of the Init 25 reads are anonymous**. Admin group-governance routes stay `current_admin`-gated (not public), consistent with the existing category-CRUD posture they replace.

**Invariants preserved:**
- `POST /tags/merge` — reused unchanged; tag-delete-requires-merge-or-empty (RESTRICT) posture intact.
- No new NFR class — reuse existing observability / i18n-parity / visual-verification / determinism NFR families (SCP § 4.1). Dark-mode is a hard AC per HANDOFF § 7 token rule.
- Anonymous share contract change is projection-only — no share-side schema migration.

### Cross-references

- PRD/Epics: `prd.md` § Initiative 25 (FR25-TAX/FILT/BROWSE/DETAIL/ADMIN/SHARE/AGENT block; supersede note prepended to FR0-CAT/ADM/SOT); `epics.md` § Initiative 25 / Epics 41–47 (E41 data model + `0018` → Decision AU/AV; E42 API → Decision AW; E43–E46 FE; E47 i18n/theming/visual/runbook cutover).
- SCP: `sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md` (§ 2 impact + diligence, § 4.2 architecture amendment, § 4.3 epic list, § 4.4 UI file map, Appendix A code evidence).
- Design: `docs/design/HANDOFF-tagi-fasetowe.md` (§ 4 UI file map, § 7 token/dark-mode rule); visual SoT `Katalog — Tagi fasetowe.dc.html`.
- Code anchors (verified 2026-07-17): `_entities.py:29-58` (Category tree), `:61-69,85-87,129-139` (Tag / Model.category_id / model_tag); `0004_entity_tables.py:73-78` (unnamed inline `category.id` FK), `:91` (named `ix_model_category_id`); `0010_drop_model_legacy_id.py:36-38` + `0005_user_uuid_audit_log.py:125-126` (batch-drop precedents); `0017_model_note_bilingual.py:28` (migration head); `sot/router.py:46-134`, `sot/admin_router.py:668-797` (`POST /tags/merge` at `:693`); `share/models.py:40` + `share/router.py:144,228`; `api-types.ts:43-63,166,183`.
- Decision lineage: retires the Initiative 0 baseline `Category` entity (v1 `docs/design/2026-04-29-portal-design.md`); reuses `model_tag` (Init 0) + `POST /tags/merge`; honours Init 6 Decision M default-deny `_PUBLIC_ROUTES` posture; follows `NFR10-SCHEMA-MIGRATION-1` forward-only precedent (Init 10 Decision L).
- Gates: **G-DEVGO** (E41 implementation BLOCKED until bmad-create-story + dev-go); **G-SEED** (starter-taxonomy seed deferred to story 41.3, out of migration `0018`); **OD-SHARE-COPY** (share detail UI copy once `category` leaves `ShareModelView` — operator confirm at handoff, not a blocker). FK/index name introspection is **RESOLVED** (Decision AV) — no open gate.

#### Update 2026-07-19 (bmad-correct-course) — category retirement re-sequenced; live API is a zero-code compatibility bridge

**Source:** SCP `sprint-change-proposal-2026-07-19-e42-deferred-coupled-cutover.md` (operator-approved re-sequencing of the Decision AW removals). This Update **does not change the target contract** — it corrects *when* and *how* the category surface retires, and fixes the `0018`/`0019` split.

- **Additive vs destructive split (see Decision AV Update).** The removals enumerated above (routes, service fns, DTOs, ORM, `ShareModelView.category`) are **NOT one change**. They are relocated out of E42 into the terminal cutover epic **E47**, in two stories:
  - **47.4 — API-surface retirement** (formerly 42.3): `GET /api/categories` + admin category CRUD + `create/update/delete_category` + `list_categories_tree` + the **route-only** schemas (`CategoryNode`/`CategoryTree`/`CategoryCreate`/`CategoryPatch`). **`CategorySummary` is NOT removed here** — it stays embedded in the retained `ModelDetail`/`ShareModelView`. Reversible by code revert.
  - **47.5 — ORM + DTO + `0019` atomic cutover** (formerly 42.5 + the orphaned ORM/`0019` drop, TB-053): drop `ModelCreate/Patch/Summary.category_id`, `ModelDetail.category`, `CategorySummary`, `ShareModelView.category`; remove `Category` validation from `create_model`/`update_model`; remove `class Category` + `Model.category_id`; ship `0019_drop_category`. **One commit** (`Model.category_id` is NOT NULL → the column drop and every DTO/ORM reader must move together). Discharges the E41 parity-test action item.
- **Zero-code compatibility bridge.** The **already-live category API, left untouched, IS the bridge** — no deprecation shim is built (minimal-diff / Ponytail-consistent). Every category removal is gated on its consumers being gone: 47.4 waits for the E43–E45 FE + 47.3 hydrate/runbook to stop calling `GET /api/categories`; 47.5 waits for 47.4 + the 45.3 create-form. `.190` auto-deploys `main` HEAD (api+web from one commit), so no removal may deploy while a live consumer remains — otherwise it is a live 404 / broken admin-add / broken hydrate, not a test failure.
- **Terminal cutover constraints (47.5).** Forward-only `0019` (`downgrade()` raises) is **irreversible by Alembic**; rollback = **verified pre-`0019` `.190` DB backup restore + `git revert` + redeploy**. The backup is a **pre-deploy** step taken under the deploy lock (`flock /tmp/3d-portal-deploy.lock`) immediately before the 47.5 merge; 47.5 is the **only** substantive change in its deploy (clean rollback); confirm a **single Alembic head** after `0019`; the **45.3 admin create-form selector removal deploys in the same `main` HEAD** as the `ModelCreate.category_id` drop. **Destructive-go is DEFERRED to 47.5 dev-go** — NOT granted by the correct-course.
- **Auth posture correction (TB-055).** The Decision AW "Default-deny auth posture" paragraph above was corrected 2026-07-19: the SoT catalog reads are **authenticated default-deny, outside `_PUBLIC_ROUTES`**; category-route removal needs **no** `_PUBLIC_ROUTES` edit; no Init 25 read is anonymous.

#### Update 2026-07-19 (bmad-correct-course) — E43 frontend types are additive; FE category removals relocated to the terminal cutover

**Source:** SCP `sprint-change-proposal-2026-07-19-e43-fe-data-additive-correction.md` (operator-approved). This Update corrects the **Frontend types** bullet of Decision AW above (already rewritten in place) so E43 stays additive and consistent with the E42→E47 relocation and the shipped 42.2 wire. It **does not change the target contract** — the category surface still retires at the terminal cutover; this fixes *what E43 touches* and *who owns the FE deletions*.

- **`TagRead.group` never existed.** The shipped 42.2 `TagRead` (`sot/schemas.py:35-47`, D-SHAPE-1) carries `group_id` + `group_position` **only**; the pre-correction "add `group`" bullet named a field the server never sends. The additive FE type adds `group_id` + `group_position` only.
- **Shipped group-read types documented.** `TagListItem` / `TagReadWithCount` / `TagGroupRead` / `TagGroupsResponse` / `TagGroupSummary` (all shipped in 42.2/42.4, `sot/schemas.py:50-101`) are now listed as the FE additive surface.
- **FE category-removal ownership split** (was wrongly folded into E43): `ModelSummary.category_id` + `ModelDetail.category` + `CategorySummary` (FE type) → **47.5** (atomic with the backend ORM + `0019` model-DTO drop, same `main` HEAD). `CategoryNode` + `CategoryTree` (FE types) + `useCategoriesTree` (hook) → **47.4** (after all E44/E45 consumers migrate; symmetric FE half of the route-only backend-schema removal). E43 removes **none** of them — the live category surface is the zero-code compatibility bridge for the FE too.

#### Update 2026-07-22 (bmad-correct-course) — 47.4 absorbed into 47.5; ModelHero breadcrumb removal added to the terminal cutover scope

**Source:** SCP `sprint-change-proposal-2026-07-22-e47-4-absorbed-into-47-5.md` (operator-approved — exact GO phrase `GO E47 ATOMIC CUTOVER`, Michał/Ezop via controller session, 2026-07-22). This Update does not change the target contract — it collapses the two-story terminal retirement into **one atomic cutover story** and adds one newly-discovered FE consumer removal to its scope.

- **Two live consumers invalidated 47.4's precondition.** Story 47.4's spec-authoring run correctly self-blocked: `AddModelForm.tsx:13,81` (admin category `<select>`, its non-migration is the documented, operator-approved 45.3↔47.5 handshake) and `ModelHero.tsx:14,65` (category-ancestry breadcrumb on `/catalog/$id` — an **undocumented** dependency; Story 45.2 scoped itself to the tag-chip row only) both still call `useCategoriesTree()`. The "consumers are gone after 44.3 + 45.x" assertion in the epic sketches was stale against shipped code.
- **Absorption.** 47.4 is absorbed into 47.5. The absorbed 47.5 executes four internal phases in one commit / one deploy HEAD, in required order: **(a)** FE consumer migration — remove `AddModelForm`'s category selector (honouring the 45.3↔47.5 handshake) and remove `ModelHero`'s category-ancestry breadcrumb **with no replacement UI** (`TagGroupsSection`, shipped in 45.2, is Initiative 25's classification surface — a replacement breadcrumb would contradict FR25-TAX-1's many-tags premise); **(b)** FE `useCategoriesTree` hook + `CategoryNode`/`CategoryTree` type deletion; **(c)** backend category route/service/route-only-schema retirement (ex-47.4 Code Map verbatim); **(d)** the original 47.5 ORM + DTO + `0019_drop_category` scope. (a)→(b)→(c)→(d): FE stops reading before the BE stops serving, before the ORM stops existing. No partial/hidden/internal category API bridge at any phase.
- **Terminal cutover constraints (47.5) — amended (strengthens the 2026-07-19 Update above; nothing loosened).** Rollback = verified pre-`0019` `.190` DB backup restore + `git revert` of the **whole absorbed cutover commit** + redeploy (partial revert would leave FE/DB inconsistent). The pre-deploy backup taken under the deploy lock (`flock /tmp/3d-portal-deploy.lock`) must not merely exist — its **restore path must be demonstrated** (scratch restore / verification query, command + output logged to `.hermes/run-logs/`) immediately before the deploy. Destructive-go was **recorded 2026-07-22** with the GO above (explicitly accepting deletion of 43 category rows and 130 model-category assignments from the live DB); restore-readiness was demonstrated pre-GO on backup `portal-pre-e47-5-20260722T172024Z.db` (evidence `.hermes/run-logs/e47-precutover-backup-20260722T172024Z.log`), which does **not** substitute for the fresh pre-deploy backup. Single Alembic head after `0019`; the cutover is the only substantive change in its deploy.

#### Forward pointer 2026-07-26 (bmad-correct-course) — Initiative 26 introduces a *different* browse entity; nothing above is reversed

**Source:** SCP `sprint-change-proposal-2026-07-26-init26-catalog-discovery.md` (direction operator-approved; SCP text ratified by Laura/controller 2026-07-26). Decisions AU/AV/AW and the three Updates above are **historically accurate and unchanged**. The mandatory single category per model (`Model.category_id` NOT NULL, self-referential `category` tree, `CategoryTree` API, `class Category`, `CategorySummary`/`CategoryNode`/`CategoryCreate`/`CategoryPatch`, `useCategoriesTree`) stays **retired permanently**; `0019_drop_category` is forward-only and is never reverted.

Initiative 26 adds a **new and different** entity — an independent, admin-curated, **many-to-many** browse category with no column on `model`, zero-membership valid, flat for MVP with a product depth ceiling of 2. To keep the two unambiguous in code and in migration history, the new entity uses **distinct physical and code identifiers** (`browse_category`, `model_browse_category`, `BrowseCategory*`); **no deleted identifier is resurrected**. See § Initiative 26 / Decisions AX–BA below.

## Initiative 26 — Catalog Discovery: Independent Browse Categories + Tag-Aware Search + Mobile Gallery Maturity

**Status:** 🚧 planning (started 2026-07-26; controller review iteration applied 2026-07-26). Source SCP: `sprint-change-proposal-2026-07-26-init26-catalog-discovery.md` — direction operator-approved via the input packet; SCP text **ratified by Laura (controller) 2026-07-26**, no Ezop sign-off recorded or implied. Epics 48–54 (E48 shipped; see `epics.md` § Initiative 26). **Change class: MAJOR** — new first-class browse entity, additive migration `0020`, a search-semantics change on a shipped endpoint, and a catalog navigation cutover. **No destructive DDL and no endpoint retirement anywhere in this initiative.**

### Overview

Initiative 25 gave the catalog one classification vocabulary — facet tags — and then asked it to do two jobs: navigation ("show me lamps") and refinement ("…with an M10 thread, in PETG"). Facet tags are right for refinement and stay exactly as shipped. Initiative 26 adds the missing browse layer, makes free-text search able to reach the tag vocabulary it already owns, and matures the mobile image viewer beyond the containment quickfix that shipped as Story 48.1.

Everything backend in this initiative is **additive and reversible**, which is a deliberate inversion of Initiative 25's shape: there is no compatibility bridge to maintain because nothing is being taken away. The single user-visible cutover is the browse IA change (E51), and it lands only after its data (E49) and state (E50) foundations are on `main`.

### Pre-enumeration save (reuse + extend, never parallel-implement)

Verified at HEAD `da87e71` before any new design was drawn:

- **`ModelTag` is the M:N join pattern** (`_entities.py:117-127`) — composite PK, `model_id` `CASCADE`, `tag_id` `RESTRICT`, plus the reverse-direction `Index("ix_model_tag_tag_model", "tag_id", "model_id")`. `ModelBrowseCategory` is this shape verbatim. Nothing new is invented.
- **`GET /api/tags?q=&limit=&with_counts=` already implements tag-name search** (`sot/service.py:82-107`, `sot/router.py:46-75`) — case-insensitive substring over `slug`/`name_en`/`name_pl`, bounded `limit` 1–200. The FR26-SEARCH-2 suggestion surface **reuses** it; **no new suggestion endpoint is authorized** without a story-level justification of why this one is insufficient.
- **The subquery-not-join filter pattern is established** — `tag_ids` (`service.py:216-254`) and `external_url` (`:267-276`) both use `Model.id.in_(select(...))` precisely to keep the outer `SELECT` shape and therefore the `total`/pagination pipeline intact. FR26-SEARCH-1 extends this pattern; it does not introduce one.
- **`TagGroup`'s explicit index naming** (`_entities.py:31-36`) records the ORM-vs-migration auto-naming drift trap. The new entities use explicit `Index(...)` names for the same reason.
- **`tag_group_admin_router.py` + the 42.4 audit posture** (`tag_group` entity_type, `tag_group.*` actions, `current_admin`, audit row per write) is the structural precedent for the admin category surface.
- **`test_orm_migration_parity.py`** (shipped in 47.5, `compare_metadata` against a migration-upgraded scratch DB) already guards ORM↔migration drift. `0020` needs **no new drift guard**, only a green parity assertion.
- **`apps/web/tests/visual/image-viewer-containment.spec.ts`** (shipped in 48.1) is a standing geometry regression suite. E53 keeps it green; it does not replace it.

### Decision AX — Browse-category data model: a new entity, deliberately not the retired one

**Proposed** 2026-07-26 for Initiative 26, per SCP § 6; amended by the 2026-07-26 controller review. Target `apps/api/app/core/db/models/_entities.py` **and** `migrations/versions/0020_browse_categories.py` — **one atomic story** (Story 49.1). The entities and their migration are **not independently mergeable**: `test_orm_migration_parity.py` (shipped in Story 47.5) asserts `alembic.autogenerate.compare_metadata` produces an empty diff between the migration-upgraded scratch DB and `SQLModel.metadata`, so an entities-only branch fails it (metadata has tables the DB lacks) and a migration-only branch fails it (DB has tables metadata lacks). Neither half can pass `check-all` 16/16 alone. This is the same coupling that produced E41 retro action item #2 and forced Story 47.5's single-commit shape — applied here **at decomposition time** rather than discovered at spec-authoring, which is exactly what the standing epic:47 action item asks for.

- **Add `BrowseCategory` (`__tablename__ = "browse_category"`):** `id` uuid PK; `slug` (unique + index — the stable public address); `name_en`; `name_pl: str | None`; `description_en: str | None`; `description_pl: str | None`; `inclusion_criterion: str | None`; `position: int = 0`; `parent_id: uuid.UUID | None` FK → `browse_category.id`, `ondelete="RESTRICT"`, nullable; `created_at`; `updated_at`. Explicit `Index("uq_browse_category_slug", "slug", unique=True)`.
- **Add `ModelBrowseCategory` (`__tablename__ = "model_browse_category"`):** composite PK `(model_id, category_id)`; `model_id` FK → `model.id` `ondelete="CASCADE"`; `category_id` FK → `browse_category.id` `ondelete="RESTRICT"`; `created_at`; plus `Index("ix_model_browse_category_cat_model", "category_id", "model_id")`.
- **`Model` gains no column.** `Tag`, `TagGroup`, `ModelTag` are byte-unchanged.

**Why this shape.** It is `ModelTag` verbatim (see the pre-enumeration save). The composite PK delivers the required unique pair and the model→category direction; the extra index delivers the category→model direction — the packet's "indexed both ways" with zero net-new pattern.

**Why `RESTRICT` on `category_id`.** It makes FR26-ADMIN-1 structural rather than procedural: a category in use cannot vanish out from under models, and delete requires an explicit audited detach first. This is the same posture as `model_tag.tag_id` (delete-requires-merge-or-empty). `CASCADE` on `model_id` matches every other model-owned child row — deleting a model must not orphan membership.

**Why the identifiers are `BrowseCategory*` and not `Category*` — load-bearing.** `class Category`, `CategorySummary`, `CategoryNode`, `CategoryTree`, `CategoryCreate`, `CategoryPatch`, `useCategoriesTree`, and the table `category` were **deleted by Story 47.5** and are referenced throughout this repo's SCPs, retrospectives and architecture history as *the mandatory single-category tree*. Resurrecting those exact identifiers with different semantics would make every historical sentence ambiguous, and would make a migration line reading `create_table("category")` immediately after `0019_drop_category` genuinely dangerous to interpret. Distinct identifiers cost nothing — `TagGroup` ↔ `tag_group` is the same explicit-naming discipline — and buy unambiguous history. **Product vocabulary is unaffected:** users, UI copy and URLs say "category" / "Kategorie".

**Depth-2 ceiling is enforced in the service layer, not in DDL.** A nullable self-FK cannot express "at most two levels" in SQLite. Admin create/update rejects a parent that itself has a parent, and rejects self-cycles (the deleted `_would_cycle` helper is prior art whose *logic* may be re-derived; the identifier is not resurrected). MVP writes no child categories at all.

**Invariants preserved:** no existing index, constraint, or column is touched; `Tag.group_id` stays a **single nullable FK** (no Tag↔TagGroup M:N — explicit owner decision); `model_tag` and the whole facet-tag contract are unchanged.

### Decision AY — API contract: additive browse reads, category scope, tag-aware search, admin governance

**Proposed** 2026-07-26, per SCP § 6. Targets `apps/api/app/modules/sot/{router,schemas,service}.py`, `{admin_router,admin_schemas,admin_service}.py`, and `apps/web/src/lib/api-types.ts`.

**Read surface — all authenticated default-deny (`current_user`), outside `_PUBLIC_ROUTES`,** exactly like every other SoT catalog read (Decision AW "Default-deny auth posture" / Init 6 Decision M). `_PUBLIC_ROUTES` (`main.py:50-61`) needs **no** edit: none of these routes is anonymous.

- **`GET /api/categories`** — flat list ordered by `(position, slug)`; each item `{id, slug, name_en, name_pl, description_en, description_pl, position, parent_id, model_count}`. `model_count` = distinct non-deleted models assigned, computed by the same shared-helper approach as `GET /api/tags?with_counts=true` so the two can never disagree. **Empty categories are returned** — they are exactly what the curation QA surface must see.
- **`GET /api/categories/{slug}`** — one category by stable slug; `404` on unknown slug.
- **`GET /api/models?category=<slug>`** — **one** category scope, addressed by **slug**, composing with `q`, `tag_ids`, `tag_match`, `untagged`, `status`, `source`, `sort`, and pagination. An **unknown slug yields an empty page with `total = 0`, not a 404** — a stale bookmark degrades gracefully, matching the shipped `tag_ids` posture where an unknown id produces an unsatisfiable predicate (`service.py:231-238`).
- **`ModelDetail` gains `categories: list[BrowseCategorySummary]`.** `ModelSummary` deliberately does **not** — list cards render no categories in the MVP IA, so adding it would mean a second per-page eager-load for no rendered pixel.

**Why slug for `category` but UUID for `tag_ids`.** The browse scope is addressed by stable slug everywhere in the product (`/categories/{slug}` is the canonical URL; `GET /api/categories/{slug}` is the detail read), so a slug param means first paint needs no slug→id resolve round-trip. Tags keep UUIDs because tag slugs are curated and mergeable via `POST /tags/merge`, and the FE already holds the full tag set from `GET /api/tag-groups`. The Story 43.3 canonical-UUID hardening for `tag_ids` (`routes/catalog/index.tsx:34,57-67`) is untouched.

**Tag-aware `q` (FR26-SEARCH-1) — membership predicate, never a join.** The `q` clause (`service.py:258-266`) gains **one disjunct inside the existing `or_()`**:

```
OR Model.id IN (
    SELECT model_tag.model_id
    FROM model_tag JOIN tag ON tag.id = model_tag.tag_id
    WHERE lower(tag.name_pl) LIKE :like OR lower(tag.name_en) LIKE :like
)
```

`IN`/`EXISTS` rather than a `JOIN` is **required, not stylistic**: a model carrying three matching tags would appear three times under a join, and since `total` is `count(*)` over `base.subquery()` computed **before** pagination (`service.py:279-280`), a join would inflate both the count and the page. The subquery keeps the outer `SELECT` shape unchanged, so the existing sort / offset-limit / eager-tag / eager-gallery pipeline applies untouched — the same argument `external_url` already records at `:267-276`. Soft-delete (`:210-211`) and every base filter are unaffected because the disjunct is added **inside** the `q` `or_()`, not alongside them. Whether `tag.slug` should also match is left to story-creation with a recommendation of **no** (slugs are internal; the packet names `name_pl`/`name_en` only).

**NFR26-PERF-1 is asserted as query count, not latency** (made measurable by the 2026-07-26 controller review). The `q` path must execute a **constant number of SQL statements** — constant across page size, and constant across the number of matching tags and matching models (no N+1 in the membership branch, the count branch, or the eager tag/gallery hydration) — and must report `total` as the **distinct** model count. Verified by comparing executed-statement counts under two fixture sizes chosen to yield **equal result counts** on the same page, so the comparison isolates fan-out from result volume. In-repo precedent: Story 42.2's "constant-query no-N+1 proven". **No wall-clock threshold is asserted** — none can be honestly baselined here.

**Suggestion surface: REUSE, not build.** `GET /api/tags?q=&limit=&with_counts=` already returns exactly what FR26-SEARCH-2 needs. **No new or duplicate suggestion endpoint is authorized.** The 6–8 cap and the bilingual dedupe are **client-side** concerns: one tag matching in both PL and EN is already a single row server-side (the match is an `or_` over columns of the same row), so dedupe is about not rendering two *labels*, not two rows.

**Where the group label in `+ Kabel · Zastosowanie` comes from — resolved, not assumed** (added by the 2026-07-26 controller review). The shipped `TagRead` wire carries **`group_id` + `group_position` only**; there is deliberately **no embedded group label** (Decision AW / D-SHAPE-1, `sot/schemas.py:35-47`) — the human label is delivered by `GET /api/tag-groups`, which the catalog already loads via `useTagGroups()` (Story 43.2). Resolution order for Story 50.3: **(1)** `VERIFY-AT-CREATE-STORY` that `GET /api/tags` still returns `group_id` on every item; **(2)** resolve `group_id` → group label from the **already-loaded** `useTagGroups()` map, adding no request on the suggestion path; **(3)** if that map is not loaded on the mounting surface, render the pill **without** the group suffix rather than blocking on a fetch — a groupless tag (`group_id IS NULL`) renders without a suffix by definition anyway; **(4)** only if the current contracts genuinely cannot supply the label may the story propose the **smallest additive contract extension** (e.g. an opt-in flag on the existing tags read) at story-creation — **never** a second suggestion endpoint, and never a per-suggestion N+1 lookup.

**Admin governance (`current_admin`, audit row on every write; structural precedent `tag_group_admin_router.py` + the 42.4 audit posture):**

- `POST/PATCH/DELETE /api/admin/categories` — create / rename+reorder+reparent / delete.
- **Delete policy:** `409 Conflict` while assignments exist. An explicit `detach=true` performs detach + delete in one transaction and writes an audit row carrying **both** the detached model ids and a count (the epic:42 unbounded-audit-payload note applies by analogy).
- `PUT /api/admin/models/{id}/categories` — **replace-set** assignment: the whole set in one idempotent call, **explicit last-writer-wins**. Chosen over add/remove deltas because the admin UI edits a set, so a replace-set matches the actual edit unit and avoids partial-merge surprises.

  **Honest concurrency posture (corrected by the 2026-07-26 controller review; the earlier claim that replace-set has "no lost-update ambiguity" was WRONG and is retracted).** Last-writer-wins **does permit a lost update.** If admin A and admin B both load a model's categories, A adds `lamps` and saves, then B — still working from the pre-A snapshot — adds `kitchen` and saves, **A's change is silently discarded**, because B's payload never contained it. Replace-set does not prevent this; it only makes the discarded state a whole set rather than a partial merge. **Accepted posture for the current single-admin deployment: explicit, auditable LWW.** The justification is not that the race is absent — it is that every write already emits an audit row carrying the resulting set, so a lost update is *recoverable and attributable after the fact* rather than invisible. **Named future trigger:** the moment a second concurrent admin editor exists, or an automated/agent writer is added, this needs optimistic concurrency — a `revision` integer or an ETag / `If-Match` precondition returning `409` on a stale write. Until that trigger fires, no revision column and no precondition header is built (minimal-diff). Any UI over this endpoint must not imply merge or conflict-detection semantics it does not have (Story 52.2).
- New audit `entity_type` `browse_category` + `browse_category.*` actions.

**Invariants preserved:** `GET /api/tag-groups`, `GET /api/tags`, `POST /tags/merge`, the Story 42.1 AND-between/OR-within semantics, `tag_match`, `untagged`, and the anonymous `ShareModelView` contract are **all unchanged**. Categories are **not** added to the share projection in MVP; NFR25-LEAKFENCE-1's negative share-DTO test stays as the fence.

**Recorded tradeoff — re-using the retired `/api/categories` path (gate G26-ROUTE-PATH).** This path was retired by Story 47.5 and today 404s. Re-use was chosen over `/api/browse-categories` because (a) it is the correct REST name for what the product calls categories; (b) there are provably **zero** live consumers — application source is clean, `infra/scripts/cutover-smoke.sh:397-405` was already re-pointed to `/api/tags` and merely *comments* on the retirement, and Story 47.3 removed the runbook pre-flight; (c) the shape differs entirely (flat list + counts vs recursive `CategoryTree`), so no caller could silently succeed against the wrong contract. **The real cost is documentation drift** — `docs/operations.md:426,463-464,613-614` currently states the surface is retired — which is why Story 54.3 is a required, not optional, story. The alternative is recorded so a ratifying reviewer can overturn this cheaply.

### Decision AZ — Migration `0020_browse_categories`: additive **and reversible**

**Proposed** 2026-07-26, per SCP § 6. `down_revision = "0019_drop_category"`. **Ships atomically with the ORM entities in Story 49.1** — see Decision AX for why the two halves are not independently mergeable.

**`upgrade()`** — structural only, **no seed content**: `op.create_table("browse_category", …)` + `op.create_index("uq_browse_category_slug", "browse_category", ["slug"], unique=True)`; then `op.create_table("model_browse_category", …)` with the composite PK + `op.create_index("ix_model_browse_category_cat_model", "model_browse_category", ["category_id", "model_id"])`. The starter category set is a **separate admin-run seed story (49.3)**, keeping content decisions out of the migration — the 41.3 precedent, adopted deliberately.

**`downgrade()`** — **implemented**: `drop_table("model_browse_category")` then `drop_table("browse_category")`. This is a **deliberate departure** from `0018` and `0019`, which raise `NotImplementedError`. Forward-only exists to make *irrecoverable data loss* honest; `0020` creates tables that did not previously exist and destroys nothing, so a raising `downgrade()` here would be cargo-cult ceremony that removes a genuinely safe rollback path. Recorded explicitly so a future reader does not "fix" it into a raise for false consistency.

**Table-name collision safety — the packet's explicit requirement.** `0019` dropped `category`; `0020` creates `browse_category`. **No name is reused**, so no restore-then-upgrade path can collide and no migration reader can confuse the two contracts. This is the concrete discharge of "must not recreate the old dropped category table contract accidentally".

**Drift guard: no new test needed.** `test_orm_migration_parity.py` (shipped in 47.5) runs `compare_metadata` against a migration-upgraded scratch DB and fails on any ORM↔migration disagreement, including the new entities.

**`VERIFY-AT-CREATE-STORY`:** `0019.downgrade()` raises, so head-downward traversals already stop there; `0020` does not change that, but the `test_migration_00xx` family must be re-checked for the same head-pinning adjustment pattern that Story 47.5's `d11` applied.

**Deploy posture.** Additive + reversible ⇒ the destructive-go protocol that governed `0019` (fresh verified backup under `flock /tmp/3d-portal-deploy.lock`, demonstrated restore-readiness, sole-substantive-change deploy, whole-commit revert plan) **does not apply**. Standing pre-deploy backup policy is sufficient. A single Alembic head after `0020` is still asserted.

### Decision BA — Mobile lightbox: adoption is a scored decision story, not a plan assumption

**Proposed** 2026-07-26, per SCP § 6. Target `apps/web/src/modules/catalog/components/imageViewer/`.

The operator packet records a **research preference** for *Yet Another React Lightbox* + Zoom plugin with *PhotoSwipe 5.4.x* as plan B, and states explicitly that **adoption is not pre-decided**. This decision therefore fixes a *procedure*, not a dependency.

**Story 53.1 must score exactly THREE options — (1) Yet Another React Lightbox + Zoom, (2) PhotoSwipe 5.4.x, (3) extend the existing in-house viewer — and produce a written, evidence-backed recommendation** across four criteria: (1) measured bundle-size delta against the current build; (2) integration cost with both existing mounts — `ModelGallery` on `/catalog/$modelId` and `ShareCarousel` on `/share/$token` — **without disturbing their `renderImage`/`renderThumb` auth boundaries** (an `Always:` constraint carried verbatim from the Story 48.1 spec); (3) accessibility — WAI-ARIA APG modal-dialog semantics, focus trap and return focus, and **visible** zoom controls satisfying WCAG 2.2 SC 2.5.1/2.5.7 without relying on pinch; (4) **physical Android Chrome gesture quality**, which synthetic Playwright touch events cannot settle. **Option 3 (extend in-house) must be scored as seriously as the two libraries**: Story 48.1 proved the component is small, understood, and now covered by a geometry regression suite. A dependency must earn its place.

**Carried-forward invariants from Story 48.1 (must not regress):**
...
**Explicitly deferred (Story 48.1's recorded residual risk, not adopted into E53 scope):** `Viewer3DModal` and other shared `DialogContent` consumers may exhibit the same mixed-reference-box pattern. E53 touches the image lightbox only; migrating other dialogs requires a reported defect or an explicit story — the boundary Story 48.1 itself drew.

### Cross-references

- PRD/Epics: `prd.md` § Initiative 26 (FR26-CAT/SEARCH/BROWSE/ADMIN/GOV/VIEW + NFR26 block; forward pointer prepended under the Initiative 25 supersede note); `epics.md` § Initiative 26 (E48 shipped; E49 data/API → Decisions AX/AY/AZ; E50 FE data+state+suggestions; E51 browse IA; E52 filters + admin curation; E53 lightbox → Decision BA; E54 i18n/a11y/visual/rollout).
- SCP: `sprint-change-proposal-2026-07-26-init26-catalog-discovery.md` (§ 1.3 code-verified evidence table, § 2.1 retired-vs-new boundary, § 6 decision text, § 8 open gates, § 9 decomposition).
- Code anchors (verified 2026-07-26 at HEAD `da87e71`): `_entities.py:29-44` (`TagGroup` explicit index naming), `:47-60` (`Tag.group_id` single nullable FK), `:117-127` (`ModelTag` join shape); `sot/service.py:82-107` (tag search — reuse), `:171-284` (`list_models`: tag predicate `:216-254`, `q` `:258-266`, `external_url` subquery rationale `:267-276`, `total` before pagination `:279-280`); `sot/router.py:46-75` (`GET /api/tags`); `main.py:50-61` (`_PUBLIC_ROUTES`); `migrations/versions/0019_drop_category.py:29-43` (forward-only head); `routes/catalog/index.tsx:34,49-103` (URL state layers); `FilterRibbon.tsx:67-72,99-113` (search input + `+tag` control); `FacetSidebar.tsx` (taxonomy-as-navigation); `infra/scripts/cutover-smoke.sh:397-405`; `docs/operations.md:426,463-464,613-614`.
- Decision lineage: extends — never reverses — Decisions AU/AV/AW (Initiative 25). Reuses the `ModelTag` M:N posture (Init 0), the Init 6 Decision M default-deny stance, the 42.4 admin-governance + audit shape, and the 41.3 seed-outside-migration precedent. Inverts NFR25-SCHEMA-MIGRATION-1's forward-only posture *by design* (NFR26-SCHEMA-ADDITIVE-1) because `0020` destroys nothing.
- Gates after the 2026-07-26 controller review: **G26-SCP-RATIFY ✅ closed** (SCP text ratified by Laura/controller; no Ezop sign-off implied) · **G26-ROUTE-PATH ✅ closed** (`/api/categories` re-use ratified together with the internal `BrowseCategory*` naming) · **G26-UXGATE ✅ decided** — a targeted `bmad-ux` pass runs before Story 50.3, Epic 51 and Epic 52; E49 and Story 53.1 are not blocked · **G26-CAT-SET 🔓 open, routed** into that same UX/taxonomy pass, before Story 49.2 · **G26-LIB 🔓 open** (Decision BA) · **G26-MIGRATE 🔓 open** (Decision AZ; explicitly **not** a destructive gate) · **G26-DEVGO 🔓 open** — planning proceeds; code starts only after create+validate and controller confirmation of that specific ready story under the standing initiative authorization.
