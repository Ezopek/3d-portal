...
initiatives:
  ...
  - id: 22
    ...
    status: 'planning'
    started: '2026-06-06'
    sections: 'see "Initiative 22" H2 below'
---
...
**Last updated:** 2026-06-06 (Initiative 22 Admin Operational Observability — Worker/Job Console for ARQ queues — planning extension via sprint-change-proposal-2026-06-06-init22-admin-jobs-console.md, status `proposed` 2026-06-06; H2 append after Initiative 21. Read-only admin MVP; implementation BLOCKED until bmad-create-story + dev-go. Prior: 2026-06-04 Initiative 21 Admin-Managed Orca Process Profiles via sprint-change-proposal-2026-06-04-profile-admin.md.)
...
| 25 | Facet Tag Taxonomy + Category Retirement (Catalog Rebuild) | 🚧 planning | — | **MAJOR** — retires the shipped `Category` entity and rebuilds catalog browse/filter/detail/admin around **facet-grouped tags**. Seven epics **E41–E47** (backend contract E41–E42 → frontend E43–E46 → cutover E47): `TagGroup` entity + `Tag.group_id` + Alembic `0018` forward-only drop of `Category`/`Model.category_id` (E41); facet filtering (`tag_ids`/`tag_match` AND-between-groups/OR-within-group + `untagged`), group governance, category-endpoint removal, `ShareModelView` category drop (E42); FE data layer + `FacetSidebar`/`FilterRibbon` browse UI + grouped card/detail/edit + admin tag/group screen (E43–E46); i18n + dark-mode token AC + pl-PL visual specs + agent-runbook cutover (E47). **No data migration** — models reset to untagged, triaged via `untagged=true`. Supersedes cancelled residuals `38-4` + `4-6`. Admin-only tag creation. Retires FR0-CAT-1..3, FR0-ADM-1, FR0-SOT-1. Source SCP: `sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md` (APPROVED 2026-07-17). Design: `docs/design/HANDOFF-tagi-fasetowe.md`. See section "Initiative 25" below. |
...
- **FR0-CAT-1.** List view + grid view of models, with thumbnails (rendered server-side by the worker). _(Superseded by Initiative 25 — facet tags: the category dimension of catalog browse is retired; see Initiative 25 for the facet-tag replacement.)_
- **FR0-CAT-2.** Free-text search across model name, tags, categories. _(Superseded by Initiative 25 — facet tags: categories retired; search spans name + facet tags. See Initiative 25.)_
- **FR0-CAT-3.** Filter by tags. Filter by categories. _(Superseded by Initiative 25 — facet tags: category filtering retired, replaced by facet-tag filtering (AND-between-groups / OR-within-group + `untagged`). See Initiative 25.)_
...
- **FR0-ADM-1.** Edit model metadata: name, description, tags, categories. _(Superseded by Initiative 25 — facet tags: the category field is retired; metadata editing is tag/facet-based (admin-only tag creation). See Initiative 25.)_
...
- **FR0-SOT-1.** Public read endpoints: list models, get model, list tags, list categories. _(Superseded by Initiative 25 — facet tags: `GET /api/categories` retired; replaced by `GET /api/tag-groups` (facet groups + tags + counts). See Initiative 25.)_
...
**Status:** 🚧 in-progress (started 2026-06-04). 33.1 + 33.2 **shipped**; **33.3 FROZEN/BLOCKED** pending profile-model correction (2026-06-06). Source SCP: `sprint-change-proposal-2026-06-04-profile-admin.md` (status `approved-but-corrected` — operator go: *"Ok, możemy ruszać z pracami"*; approval is scoped to planning-artifact appends, NOT to code implementation per SCP § 7). Init 21 gives the operator a first-class **admin panel to see, import, and manage Orca process/intent profiles**, and makes the **user-facing Files/STL selector expose only admin-approved + material-compatible options**. It builds directly on Initiative 20 (the resolver + the EST-TIERS-1 availability bridge) and supersedes the EST-TIERS-1 hand-placement workaround: the per-tier availability signal stops being "derived purely from disk presence" and becomes "derived from the admin-managed, compatibility-constrained profile inventory" — while the user-facing `GET /api/estimates/quality-tiers` `{quality_tier, available, reason}` contract is preserved. Single epic E33, **read-only inventory first**. Predecessor: Initiative 20 (STL Slicer Estimates, Epic E32 — shipped). Kanban continuity card: `t_ce1927cf`.

> ⚠️ **DOMAIN-MODEL CORRECTION (2026-06-06) — read before further Init 21 work.** Source: `sprint-change-proposal-2026-06-06-epic33-profile-model-correction.md`. The fixed `printer_ref × material_class × quality_tier` slot grid + fused intent triple this section describes is **NOT the canonical domain model** — it is a **transitional compiled-intent / resolver projection** (kept as MVP compatibility for the existing resolver/worker, **not deprecated**). The **canonical model is separate Orca-like building blocks**: **`MachineProfile`** (required for valid Orca slicing — a real current Orca machine, e.g. K1 Max / MicroSwiss HF; may later carry machine-capability material restrictions), **`ProcessProfile`** (**the most important portal-facing block** — it drives gram/material estimates, print strategy, and the member quality/process choice), **`FilamentProfile`** (needed by Orca; drives **max volumetric speed**, compatibility, time estimates; **bridges to Spoolman material types**) — plus **`ProfileChain`** (the compiled intent the existing resolver consumes) and **`PrintProfileOffer`** (member/admin offer: label, visibility/default, compatible material categories, optional concrete-filament overrides, representative compiled chain). Shipped 33.1/33.2 are **preserved, not reverted** (reclassified as the compiled-projection read/write); their safety foundations carry forward. **Story 33.3 (grid lifecycle) is FROZEN/BLOCKED**; the next slice is re-pointed to a small **profile-library inventory CRUD** (add/import, list, get curated metadata + validation state, delete — **process profiles first**, machine/filament supporting refs; **no raw Orca JSON viewer**) → then `PrintProfileOffer`/`ProfileChain`. No data migration is forced (the block/offer layer compiles into the same resolver intent path; `bundle_hash` / append-only stores / provenance snapshots preserved). Material taxonomy stays a small generic bridge (PLA/PETG/PCTG/TPU, later ABS/ASA); member spool-picker / request-flow UX is deferred. FR21-*/NFR21-* below remain valid for the shipped compiled-projection surface; future block/offer requirements are authored when that slice is sequenced.
...
**Status:** 🚧 planning (started 2026-06-06). Source SCP: `sprint-change-proposal-2026-06-06-init22-admin-jobs-console.md` (status `proposed` 2026-06-06 — approval is scoped to planning-artifact appends, NOT to code implementation). Discovery + architecture/UX design: `_bmad-output/implementation-artifacts/spec-admin-jobs-console.md` (candidate id **ADMIN-JOBS-1**, committed `dcb9df8 docs: design admin jobs console`). Init 22 gives the operator a **read-only, admin-only worker/job console** over the three live ARQ pools so that after changing STL/render inputs or profile offers they can see whether a worker woke and whether jobs are **queued / running / succeeded / failed**. Single epic E34, **single read-only MVP story**. Predecessor: Initiative 21 (Admin-Managed Orca Process Profiles, Epic E33) — whose slice/recompute queues this console observes. **Implementation BLOCKED** until `bmad-create-story` authors the numbered story spec and the operator's dev-go gate fires.

### Overview

The portal runs **three live ARQ worker pools** — API (`arq:api`), Render (`arq:queue`), Slicer (`arq:slicer`) — and there is **no in-product way to observe them**: `GET /api/health` (`apps/api/app/main.py:200-202`) returns only `{status,version}` and never touches arq. Init 22 adds a backend snapshot endpoint **`GET /api/admin/queues`** and a frontend **`/admin/queues`** admin tab that answer three operator use cases: **UC1** "did my change wake the backend?" (queued → running), **UC2** "why hasn't anything changed after 5 minutes — is something running ahead of mine?", **UC3** "what are these red jobs?" (failures legible enough to discuss). The console is built strictly by **reusing existing seams** — the FastAPI-lifespan-owned arq pool + raw Redis on `app.state` (`apps/api/app/main.py:64-94`), the arq 0.28.0 read surface, and the existing business-keyed status keys (`render:status:{model_id}`, slicer `EstimateStatus`) as a job-context layer — never an ad-hoc Redis connection and never re-implementing arq.

It is deliberately **read-only and admin-only**: no retry / kill / purge / pause / resume controls (those are deferred destructive operator-action slices); no raw Redis/ARQ internals dump as the product surface; no WebSocket/SSE; not the member-facing `/queue` print-queue module slot (a semantically different v2 surface — the console lives in the admin area). The MVP ships a **raw-arq live snapshot** (exact `zcard` queued depth + bounded `SCAN` for in-progress + bounded, hard-capped `SCAN` for recent results — **never** the unbounded `KEYS` that arq's `all_job_results()` uses); the **durable job-activity ledger is deferred** as a named later slice (G-LEDGER).

### Functional Requirements

- **FR22-QUEUE-SNAPSHOT-1** (backend read — Story 34.1): An admin-only `GET /api/admin/queues` returns a read-only snapshot computed entirely from `app.state.arq` / `app.state.redis`: per-pool `queued` (`redis.zcard(queue_name)`, exact O(1)); `worker` liveness + counters parsed from the `<queue>:health-check` string; a bounded `running_jobs` list from `SCAN MATCH arq:in-progress:*` (cardinality ≤ total concurrency); a bounded, hard-capped `recent` list from `SCAN MATCH arq:result:*` (**never** unbounded `KEYS`), each projected to allowlisted fields only and grouped by `JobResult.queue_name`; and a `retention_note` marking `recent[]` as Redis-resident (~1h). Mounted under `/api/admin` (OD-4: new `apps/api/app/modules/queue/` package), `current_admin`-gated, absent from `_PUBLIC_ROUTES`. **Verifiable:** backend pytest (fakeredis) seeding `arq:*` keys and asserting the DTO; a bounded-SCAN-not-`KEYS` assertion; 403-non-admin + 401-anonymous.
- **FR22-LIVENESS-1** (worker liveness — Story 34.1): Per pool, worker liveness is a **tri-state derived from the `<queue>:health-check` key's presence + age**, honestly labelled: `alive` (present, age < interval), `idle/stale` (present, age ≥ interval), `unknown/down` (absent). The raw heartbeat counters (`j_complete/j_failed/j_retried/j_ongoing/queued`), the heartbeat timestamp, and the worker's actual `interval_s` (read from `health_check_interval`, 3600s today — **not** a chosen constant) are surfaced verbatim so coarse granularity is honest. **Verifiable:** pytest asserting the tri-state mapping over seeded health keys of varying age/presence; `interval_s` reflects the seeded value, not a hardcoded number.
- **FR22-CONSOLE-UI-1** (frontend — Story 34.1): A new `"queues"` admin tab (`AdminTabs.tsx`) + `routes/admin/queues.tsx`, mirroring the profiles-tab pattern, renders: **per-queue cards** (queued/running headline + a liveness chip showing age + interval + raw counters, failed-count visually weighted); a **"running now" strip** (function + queue + age + curated context); and a **"recent (~last hour)" list** with the **retention caveat label**, filterable by queue/outcome, curated `error_class` on failures. States: loading skeleton, empty ("no jobs queued"), and endpoint error — the console **fails closed/visible** (never fabricates green/empty on a failed read). Focus-gated polling (no WebSocket/SSE). AuthGate discipline per the Init 10 retro rule (defer to shell for anonymous; role-tier redirect only for authenticated-non-admin). **Verifiable:** vitest on the card/strip/list states + loading/empty/error (fails-closed) + focus-gated polling start/stop; Playwright baselines for populated/empty/error.

### Non-Functional Requirements

- **NFR22-LEAK-FENCE-1** (load-bearing): The DTO carries a **field allowlist, not a denylist** — `queue_name`, friendly `role`, `function`, `job_id`, counts, timings/ages, `success/outcome`, liveness, and a **curated `context`** only. It **never** serializes raw pickled `args` / `kwargs` / `result`; `error_class` is a curated category (exception type name at most, never a traceback/message); `context.ref` is an id / hash-prefix, never a filesystem path. The console avoids `queued_jobs()` (which deserializes args) — depth via `zcard`, function name only via `Job.info()` for the bounded in-progress set. **Verifiable:** a fence test asserts the serialized response matches the allowlist and contains no `args`/`kwargs`/`result` keys; a negative test asserts no path-like substrings (`/`, drive letters, `..`) and no secret-looking material (mirrors the 33.1 AC-9 provenance-fence pattern).
- **NFR22-AUTH-1**: The route is admin-gated via the pre-baked `current_admin` default-value dependency (`apps/api/app/core/auth/dependencies.py:76`) and absent from `_PUBLIC_ROUTES`, satisfying the Init 6 mechanical route-enforcement gate (`apps/api/tests/test_route_enforcement_gate.py`) with **no `_PUBLIC_ROUTES` edit**. **Verifiable:** the route-enforcement pytest passes with the new route present; 403-for-non-admin + 401-for-anonymous tests exist.
- **NFR22-READONLY-1**: The endpoint is **`GET` only** — no enqueue, abort, purge, pause, or resume. The console cannot mutate queue state. Operator-action controls are deferred (G-ACTIONS). **Verifiable:** no mutating verb/route in the queue module; a test asserts the module exposes only the read endpoint.
- **NFR22-REDIS-LOAD-1**: No unbounded `KEYS` in any request path — depth via `zcard`, in-progress + recent via bounded `SCAN` with `MATCH … COUNT <small>` and a hard cap on `recent[]`; the FE polls on a bounded interval and **pauses when the tab/document is hidden** (focus-gated). The polling interval and `recent[]` SCAN cap are arbitrary-but-bounded product defaults (magic-constant discipline: each points to its contract — UC1/UC2 watchability + homelab single-Redis load bound — not to "feels right"). **Verifiable:** a test asserts no `KEYS` call in the snapshot path; vitest asserts polling stops on `visibilitychange` hidden.
- **NFR22-RETENTION-HONESTY-1**: The `recent[]` panel is **explicitly labelled "last ~1h, Redis-resident"** so a disappeared failure (expired with arq's `keep_result` ~1h TTL) is not misread as "resolved." This honesty is the bridge until the durable ledger lands (G-LEDGER). **Verifiable:** vitest asserts the retention label renders on the recent panel.
- **NFR22-I18N-PARITY-1**: All new `modules.admin.queues.*` i18n keys are present in BOTH `en.json` and `pl.json` with correct Polish diacritics. Queue ids and function names stay **untranslated** (technical identifiers). **Verifiable:** an i18n key-parity test (en/pl).
- **NFR22-VISUAL-VERIFICATION-1**: New visual baselines for the populated console (mixed liveness + a failed job), empty, and error states across the 4 Playwright projects (desktop/mobile × light/dark), API stubbed in `api-stubs.ts`, with `baseline-reviewed:` sign-off per PNG. The new admin tab ripples the `AdminTabs` visual baselines (regen per [[reference_web_routetree_regen]]).
- **NFR22-DETERMINISM-1**: Carries forward the determinism contract — vitest + pytest 3× consecutive identical pass counts before merge.

### Decisions

- **Decision AO** (architecture): Admin queue-console **read model + location + ledger-deferred** — a read-only raw-arq **live snapshot** computed per-request over the lifespan-owned `app.state` arq/Redis (no new table, no worker change), with the existing business-keyed status keys reused as the job-context layer; the durable job-activity ledger is an explicit **deferred** slice (G-LEDGER). The console lives in the **admin area** (`/admin/queues`), NOT the member `/queue` module slot. See `architecture.md` § Initiative 22 Decision AO.
- **Decision AP** (architecture): **Worker-liveness tri-state from the coarse health key** — liveness is derived from `<queue>:health-check` presence + age (interval 3600s today, surfaced verbatim); **coarse (~1h) liveness is accepted for the MVP** because the running/queued signals UC1/UC2 need are exact regardless; lowering `health_check_interval` (worker-config + redeploy) is deferred (G-LIVENESS). See `architecture.md` § Initiative 22 Decision AP.
- **Decision AQ** (architecture): **Leak fence / read-only privacy contract** — field-allowlist DTO, never raw pickled `args`/`kwargs`/`result`, curated `error_class`/`context`, admin-only, `GET`-only. The load-bearing safety contract; each clause becomes an AC + negative test. See `architecture.md` § Initiative 22 Decision AQ.

### Open decisions — all RATIFIED to safe default by the controller (2026-06-06)

The discovery spec surfaced OD-1..OD-5, each with a safe evidence-backed default; the controller ratified every default in the task brief (no real blocker found):

- **OD-1 — RATIFIED: MVP = raw-arq snapshot + reuse existing business-keyed status keys; durable ledger DEFERRED** (Decision AO; G-LEDGER). UC1/UC2 need only live state (exact); UC3 within arq's ~1h window + the status-key context layer.
...
- **OD-3 — RATIFIED: label `recent[]` "~last hour, Redis-resident"** (NFR22-RETENTION-HONESTY-1).
- **OD-4 — RATIFIED: new `apps/api/app/modules/queue/` package** as the backend module home (the admin-observability mirror of the v2 `queue` slot) — endpoint `GET /api/admin/queues`.
- **OD-5 — RATIFIED: single read-only MVP story (ADMIN-JOBS-1)**; the durable ledger + operator-action controls are later epic stories.

### Out of scope (intentional; deferred with named triggers)

- **Durable job-activity ledger** — DB-backed append-only event model surviving past Redis TTL; worker instrumentation + Alembic. **G-LEDGER.** Closes UC3 "discuss it days later" fully.
- **Operator-action controls** — retry / kill (abort) / purge / pause / resume (destructive queue mutations; arq supports abort, so feasible later). **G-ACTIONS.**
- **Worker `health_check_interval` lowering** — near-real-time liveness; worker-config + redeploy. **G-LIVENESS.**
...
- **Member-facing print `/queue` module** — unrelated v2 slot; not touched by this initiative.

### Cross-references

- Predecessor initiative: Initiative 21 — Admin-Managed Orca Process Profiles (Epic E33) — its slice/recompute queues are what this console observes. **Sequenced BEFORE G-PUBLISH** (Init 21 PROFILE-OFFER-1 real-resolver publication / live slicing) so the operator can observe queue effects before publishing begins.
- Discovery artifact: `_bmad-output/implementation-artifacts/spec-admin-jobs-console.md` (ADMIN-JOBS-1, commit `dcb9df8`).
- Source SCP: `sprint-change-proposal-2026-06-06-init22-admin-jobs-console.md` (status `proposed` 2026-06-06).
...
**Status:** 🚧 planning (started 2026-07-17). Source SCP: `sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md` (status **APPROVED** by operator 2026-07-17 — routed Path A: PM/Architect → Sprint Planning). Design source of truth (visual): `docs/design/HANDOFF-tagi-fasetowe.md` (7 mockups + edge states, PL/EN + light/dark). Seven epics **E41–E47** (epics 41+; highest shipped is Initiative 24 / epic-40, production baseline frozen 2026-07-16). Architecture: `architecture.md` § Initiative 25 (data model + migration `0018` + API contract). **Change class: MAJOR** — retires a shipped core entity (`Category`), requires a forward-only schema migration, and amends the catalog browse/filter/detail/admin/share contract.

### Overview

The catalog was built on a **single, hard, mandatory category per model** (`Model.category_id` NOT NULL, FK → `category.id`, organised as a self-referential tree). Operating it exposed the classic taxonomy failure: one model legitimately belongs to several axes at once (a *type*, a *room*, a *system*, a *material*, a *creator*), and a single tree forces a lossy "pick one" plus deep drill-downs. Initiative 25 replaces the hard single category with **many tags grouped into facets**: a tag belongs to one admin-managed **group/facet** (Type, Room, System, Material, Creator, …), tag creation is **admin-only** (users pick from a curated set), and filter combination is a **user-visible AND/OR toggle** (default: AND-between-groups / OR-within-group). The many-to-many `model_tag` already exists and is reused. **No data migration:** old categories are dropped, every model starts **untagged** and is re-tagged from scratch (deliberate owner reset), with `untagged=true` as the post-cutover triage filter. This supersedes and finalises the two residuals cancelled in the frozen baseline (`38-4-member-offer-request-estimate-cta`, `4-6-add-model-from-url-cli`).

> **Supersede note (applied to FR0-CAT-1..3, FR0-ADM-1, FR0-SOT-1 above):** Categories are retired by Initiative 25 (facet tags). Those `category_*` requirements are historical; the facet-tag replacement is defined by the FR25 block below.
>
> **Forward pointer (added 2026-07-26 by `bmad-correct-course`, SCP `sprint-change-proposal-2026-07-26-init26-catalog-discovery.md`):** the supersede note above remains true exactly as written — the **mandatory single category per model** (`Model.category_id` NOT NULL, self-referential `category` tree) is retired permanently and is never restored. Initiative 26 introduces a **different, new** entity: an independent, admin-curated, **many-to-many** browse category (zero categories per model is valid; no column on `model`; flat MVP with a product depth ceiling of 2). Read FR26-CAT-* as new design, never as restoration of FR0-CAT-*.
...
- **FR25-SHARE-1 — Share projection drops category.** `ShareModelView` no longer carries `category`; tags continue to flow to anonymous visitors (`ShareModelView.tags: list[str]` already exists — diligence finding SCP §2.4.1).
...
No new NFR class is required — Initiative 25 reuses the existing observability / i18n-parity / visual-verification / determinism NFR families established by prior initiatives:

- **NFR25-I18N-1** (reuse): all new `catalog.facets.*` / `catalog.filters.matchAll|matchAny|untagged|noTags` / `tags.groupless` / admin merge-rename-newGroup-duplicates keys present in both `en.json` and `pl.json`; retired keys (`catalog.filters.category`, `openCategories`, `a11y.allCategories`) removed.
- **NFR25-DARKMODE-1** (hard AC per HANDOFF §7 token rule): all new facet surfaces are token-only styled and pass light + dark theme; no color literals (existing ESLint/Stylelint color-literal ban applies).
- **NFR25-VISUAL-1** (reuse): Playwright visual baselines for the new facet surfaces (sidebar, filter ribbon, grouped detail, admin screen, untagged states), rendered in **pl-PL locale** per project memory; consolidate any overlapping `/api/*` route mocks into a single handler.
- **NFR25-DETERMINISM-1** (reuse): 3× consecutive identical pytest + vitest pass counts before merge of any story.
- **NFR25-SCHEMA-MIGRATION-1** (reuse of NFR10-SCHEMA-MIGRATION-1): migration `0018` is **forward-only** (`downgrade` raises `NotImplementedError`); old revision kept for emergency revert; categories + per-model assignment are unrecoverable by design.
- **NFR25-LEAKFENCE-1** (reuse): the anonymous `ShareModelView` contract must not regress — dropping `category` must not expose any new internal field; verified by a share-DTO test.

### Decisions

- **Decision (owner, 2026-07-16):** replace the hard single category per model with many admin-managed facet-grouped tags; admin-only tag creation; user-visible AND/OR filter (default AND-between / OR-within); **no data migration** (models reset to untagged, triaged via `untagged`). Ratified into the PRD via this Initiative-25 FR block.
- **Architecture (§ Initiative 25):** `TagGroup` table + `tag.group_id` nullable FK (`ondelete="SET NULL"`) + `group_position`; remove `Category` + `Model.category_id`; Alembic `0018_facet_tags_drop_category` (`down_revision = "0017"`, forward-only, `batch_alter_table` on SQLite for the FK+index drop); API contract change on `GET /api/models` (tag_ids/tag_match/untagged), new `GET /api/tag-groups`, removal of `GET /api/categories` + admin category CRUD, `ModelCreate`/`ModelPatch` drop `category_id`, `ShareModelView` drop `category`. Exact autogenerated FK/index names for `model.category_id` to be introspected at implementation time — do **not** guess.

### Out of scope (intentional)

- **Data migration of existing category assignments** — none; deliberate owner reset to untagged (the `untagged` filter triages the post-cutover state).
- **Backward `downgrade` of migration 0018** — forward-only by design; categories unrecoverable.
- **`slicer/*` "category"** — false positive (`reason_category` / warning `code` strings, no `Category` import); explicitly untouched (SCP §2.4.2).
- **Tag-merge as new build** — `POST /tags/merge` already exists (`admin_router.py:693`); Initiative 25 **reuses** it. New admin work is **group governance** only.
- **Fuzzy sidebar search / new search endpoint** — client-side substring over already-fetched groups only.

### Cross-references

- Source SCP: `sprint-change-proposal-2026-07-17-tag-taxonomy-catalog-rebuild.md` (APPROVED 2026-07-17).
- Design (visual SoT): `docs/design/HANDOFF-tagi-fasetowe.md` (mockups 02–08 + edge states; `Katalog — Tagi fasetowe.dc.html`).
- Architecture: `architecture.md` § Initiative 25 (data model + Alembic `0018` + API contract).
- Epics: `epics.md` § Initiative 25 (Epics E41–E47, stories per SCP §4.3).
- Sprint status: `_bmad-output/implementation-artifacts/sprint-status.yaml` — epics 41–47 to be seeded by `bmad-sprint-planning` (NOT modified by this PM handoff).
- Supersedes: cancelled residuals `38-4-member-offer-request-estimate-cta` + `4-6-add-model-from-url-cli` (named in the frozen baseline).
- Process notes: dependency order is backend contract (E41–E42) → frontend (E43–E46) → cutover (E47); apply [[feedback_scp_pre_enumeration_phase]] discipline to each story spec; apply [[reference_web_routetree_regen]] if any FE story adds or changes frontend routes; visual specs render pl-PL per [[web-visual-tests-render-pl-pl]].

## Initiative 26 — Catalog Discovery: Independent Browse Categories + Tag-Aware Search + Mobile Gallery Maturity

**Status:** 🚧 planning (started 2026-07-26). Source SCP: `sprint-change-proposal-2026-07-26-init26-catalog-discovery.md` (direction **operator-approved** via the input packet `/tmp/3d-portal-init26-correct-course-input.md`, 2026-07-26; SCP text **ratified by Laura/controller 2026-07-26** after reading the artifacts under that packet and the standing `działaj` delegation — **no Ezop sign-off recorded or implied**). Epics **E48–E54** (E48 already **shipped** — see below). Architecture: `architecture.md` § Initiative 26 (Decisions AX–BA). **Change class: MAJOR** — introduces a new first-class browse entity, an additive schema migration (`0020`), a search-semantics change on a shipped endpoint, and a catalog navigation cutover.

**Predecessor:** Initiative 25 (E41–E47, closed 2026-07-23) retired the mandatory single-category taxonomy and shipped facet tags. Initiative 26 does **not** reverse any part of it.

### Overview

Initiative 25 was correct and stays: facet tags are the right vocabulary for **refinement** ("PETG, M10 thread, kitchen"). Operating the result exposed a different gap — the same taxonomy was also made to serve as **navigation**, and a full facet tree is a poor answer to "what kind of thing do I want to browse?". Two further gaps surfaced from hands-on use: free-text search reads only `name_en`/`name_pl`/`slug`, so the one input users reach for first cannot reach the tag vocabulary at all; and the fullscreen image viewer, while now correctly contained on mobile (Story 48.1), still cannot zoom or pan, so a detailed or panoramic photo cannot actually be inspected on a phone.

Initiative 26 adds the missing layers, all additively:

- a **new, independent, curated browse category** — many-to-many, zero-valid, flat for MVP, governed separately from `TagGroup` and never generated from it;
- **tag-aware free-text search** plus **inline structured suggestions** that keep the plain-query action and the `+tag` action semantically and visually distinct;
- a **browse-vs-filter IA split** — categories in navigation, facets in a `Filters (n)` surface;
- a **mature mobile lightbox** with gestures *and* single-pointer alternatives, planned as a separate epic from the already-shipped containment quickfix.

Three framing choices set by the operator packet and carried into the FRs below:
- **Categories answer "what kind of thing?"; tags answer "what exact properties?"** These are different questions and get different surfaces. No automatic synchronization or membership inference from tags in MVP.
- **`Tag.group_id` stays a single nullable FK.** No Tag↔TagGroup many-to-many. Null remains the curation-queue state and becomes visible admin hygiene.
- **Nothing is destructive.** No migration in this initiative drops anything, and no endpoint is retired. The whole initiative is additive; the only user-visible cutover is the navigation change in E51, which lands after its data and state foundations.

**Delivered ahead of plan — Story 48.1 (Scope A phase 1).** The mobile fullscreen containment quickfix shipped 2026-07-26 as commit `da87e71` (merged to `main`, deployed; `check-all.sh` 16/16, native BMAD APPROVE, independent Aider APPROVE — evidence in `spec-e48-1-mobile-fullscreen-containment.md`). It is recorded here as **Epic 48 / Story 48.1, done**, and is **not re-planned, re-designed, or re-implemented** by any Initiative 26 story. Its identifier is preserved rather than renumbered because it is already written into a merged commit's artifact. The full gesture feature is a separate epic (E53).

### Functional Requirements

- **FR26-CAT-1 — Independent curated browse categories.** A category is a first-class, admin-curated browse entity with a stable slug, bilingual labels, optional bilingual descriptions, an inclusion criterion, and an explicit `position`. It is **not** a `TagGroup`, is **not** mechanically generated from tag data, and its governance/admin surface is separate. *Verifiable:* an admin creates a category with no tag-side change; `GET /api/categories` returns it; `GET /api/tag-groups` output is unchanged.
- **FR26-CAT-2 — Many-to-many membership; zero is valid.** Model↔Category is many-to-many. A model with **zero** categories is DB-valid and stays public/visible on every user-facing surface; only the admin surface flags it as `Uncategorized / needs curation`. `Model.category_id` is **never** reintroduced. *Verifiable:* a zero-category model appears in `GET /api/models` and renders normally; admin lists it in the curation queue.
- **FR26-CAT-3 — Curation norm is advisory.** 1–3 categories per model is a **warning-level** norm. There is no hard database maximum and no write-blocking enforcement. *Verifiable:* assigning 5 categories succeeds and produces a warning, not a 4xx.
- **FR26-CAT-4 — Hierarchy: flat MVP, depth-2 ceiling.** Browse UI is flat with approximately 6–10 broad categories. The schema may carry a nullable `parent_id`; the **product** maximum depth is 2 (root + child), exactly one parent per category, never a DAG. Child-category UI ships only when real catalog distribution demonstrates the need; there is never a third level. *Verifiable:* a depth-3 parent assignment is rejected; MVP renders no child level.
- **FR26-SEARCH-1 — Tag-aware free-text search.** `GET /api/models?q=` additionally matches models whose assigned tags contain the query in `Tag.name_pl` or `Tag.name_en` (case-insensitive substring), preserving correct `total`, pagination, soft-delete filtering, and combination with category scope and `tag_ids`/`tag_match`/`untagged`, **without duplicate rows or count inflation**. *Verifiable:* `q=kabel` returns both name-matching models and models tagged `Kabel`/`Cable`; a model matching on both name and tag appears exactly once; `total` equals the distinct model count.
- **FR26-SEARCH-2 — Inline structured suggestions with distinct semantics.** Typed text remains ordinary `q` and **Enter never silently converts text into a tag**. Matching tags across PL+EN render as visually distinct pills (e.g. `+ Kabel · Zastosowanie`); selecting one stores the canonical `tag_id`, deduplicated by ID across bilingual labels, rendering the active-locale label with an optional subtle matched alias. The combined suggestion list is capped at **6–8** items on mobile and desktop with no internal scrollbar. The ordinary search action and the `+tag` action have distinct semantics, appearance, and accessible names. *Verifiable:* Enter on a tag-matching query performs a text search; the `+tag` pill carries its own accessible name; the list never exceeds 8 items; a tag matching in both languages appears once.
- **FR26-BROWSE-1 — Navigation shows categories, not the taxonomy.** Desktop left navigation lists only broad categories with optional counts. Mobile gets a separate Browse surface/menu. Facets and tags move into a `Filters (n)` drawer/panel; at most 2–4 promoted groups may be surfaced outside it, and only when justified; full tag search remains inside Filters. *Verifiable:* the default `/catalog` left rail renders categories only; every tag group stays reachable within one interaction from `Filters`.
- **FR26-BROWSE-2 — Category is a browse scope, not another filter.** The active category renders as a **scope** above the results (chip), never as another checkbox, and is **excluded** from the `Filters (n)` count. Public MVP allows exactly **one** active category scope at a time, while a model may appear under many category pages. Canonical URL `/categories/{stable-slug}`; `q`, `tag_ids`, `tag_match`, `sort` remain query params and independent visible URL state layers. A search started inside a category stays scoped by default, with a visible category chip and a one-click **"Search entire catalog"** escape. *Verifiable:* the `Filters (n)` badge does not change when a category is active; the escape control clears only the scope.
- **FR26-BROWSE-3 — Facet semantics unchanged.** OR within one `TagGroup`, AND between `TagGroup`s, with `tag_match` as the user override. Category scope is **never** mixed into `tag_match`. *Verifiable:* the shipped Story 42.1 semantics tests pass unmodified with a category scope applied.
- **FR26-ADMIN-1 — Category governance surface.** Admin CRUD, reorder, and **replace-set** assignment of a model's categories, with an audit row on every write. Delete is blocked while assignments exist unless an explicit, audited detach is requested. *Verifiable:* deleting a used category without the detach flag returns a conflict, never a silent cascade.
- **FR26-ADMIN-2 — Curation QA surfaces.** Admin QA surfaces models with zero or unusually many categories, empty or tiny categories, categories behaving like narrow tags, confusing Category/Tag label overlap, and ungrouped user-facing tags (`Tag.group_id IS NULL`). No automatic synchronization or membership inference from tags in MVP; any suggestion is advisory only.
- **FR26-GOV-1 — Category admission criteria.** Every category carries a one-sentence inclusion criterion, positive and boundary examples, bilingual labels, a stable slug, and evidence that it is a useful entry point for **multiple** models. Similar Category/Tag labels are permitted only with an explicitly recorded semantic distinction.
- **FR26-VIEW-1 — Mature mobile image viewing.** The fullscreen viewer supports pinch zoom, pan, and double-tap, **plus visible Zoom In / Zoom Out / Reset controls** as single-pointer alternatives; a stable toolbar outside the transform layer; body scroll lock with restoration; focus trap, Escape, and return focus; safe-area and dynamic-viewport handling; and explicit swipe-vs-pan conflict rules. *Verifiable:* the Story 53.3 test contract passes, including a physical Android Chrome smoke.

### Non-Functional Requirements

No new NFR class is required — Initiative 26 reuses the established observability / i18n-parity / visual-verification / determinism families, with one genuinely new accessibility requirement:

> **Ownership rule (set by the 2026-07-26 controller review, binding on all four quality NFRs below).** Every story that ships a user-visible surface **owns, at its own merge gate**, the new i18n keys, the component-level a11y assertions, and the targeted unit/visual coverage that surface requires. This is not a preference: a story branch must pass `infra/scripts/check-all.sh` 16/16 on its own, so a surface shipped without keys or coverage cannot merge. **Epic 54 is the final cross-surface parity/a11y/visual audit and remediation pass — it is never the first place proof appears.**

- **NFR26-I18N-1** (reuse): every new key present in both `en.json` and `pl.json` with genuine, non-identical Polish translations, with an explicit key-set diff **at the shipping story's close**. Story 54.1 audits cross-surface parity and terminology consistency and remediates residue; a finding there is a defect in the story that shipped the surface, not new end-of-initiative scope.
- **NFR26-A11Y-1** (**new**): every new surface satisfies WCAG 2.2 SC 2.5.1 Pointer Gestures (no path requiring a multipoint or path-based gesture), SC 2.5.7 Dragging Movements (no drag-only path), and SC 2.5.8 Target Size Minimum (≥24×24 CSS px; ≥44×44 for the fullscreen close control per the packet). The `+tag` action and the plain-search action must be distinguishable by **accessible name**, not by appearance alone. Proven per story at its own gate; Story 54.2 audits the end-to-end journey (including keyboard-only and screen-reader traversal *between* surfaces, which no single component test covers) and remediates.
- **NFR26-DARKMODE-1** (reuse, hard AC): all new surfaces token-only, light + dark, no color literals — enforced at each shipping story's gate, audited cross-surface by 54.2.
- **NFR26-VISUAL-1** (reuse): each UI story ships the targeted **pl-PL** Playwright coverage its own surface needs, with an explicit `toBeVisible()` before every screenshot. Story 54.2 owns cross-surface baseline consistency and the overlapping `/api/*` route-mock consolidation pass.
- **NFR26-DETERMINISM-1** (reuse): 3× consecutive identical pytest + vitest pass counts before merge of any story.
- **NFR26-SCHEMA-ADDITIVE-1** (**contrast with NFR25-SCHEMA-MIGRATION-1**): migration `0020_browse_categories` is **additive and reversible** — `downgrade()` is implemented, because nothing pre-existing is destroyed. No destructive DDL anywhere in Initiative 26, and therefore no destructive-go protocol. Single Alembic head after `0020`. The migration ships **atomically with the ORM entities in one story**, because `test_orm_migration_parity.py` (`compare_metadata`) fails on either half alone.
- **NFR26-PERF-1** (**new; made measurable by the 2026-07-26 controller review**): the tag-aware `q` path must execute a **constant number of SQL statements** — constant across page size, and constant across the number of matching tags and matching models (no N+1 in the tag-membership branch, the count branch, or the eager tag/gallery hydration) — and must report `total` as the **distinct** model count. Verified by comparing executed-statement counts under two fixture sizes chosen to yield **equal result counts** on the same page, so the comparison isolates fan-out from result volume. In-repo precedent: Story 42.2's "constant-query no-N+1 proven". **No wall-clock latency threshold is asserted**, because none can be honestly baselined here; the predicate stays a membership (`IN`/`EXISTS`) subquery, never a row-multiplying join.

### Decisions

- **Decision (owner, 2026-07-26):** restore Category as a **new independent curated browse entity**, many-to-many with Model, never as `Model.category_id` and never as a special `TagGroup`; zero categories is valid and the model stays public; 1–3 per model is an advisory norm; flat browse MVP with a product depth ceiling of 2; Category and TagGroup governance remain separate. Ratified into the PRD via this FR26 block.
- **Decision (owner, 2026-07-26):** keep `Tag.group_id` as a single nullable FK — no Tag↔TagGroup many-to-many. A word that appears to fit two groups is resolved by choosing dominant semantics or by creating genuinely distinct semantic tags, never by duplicated aliases.
- **Decision (owner, 2026-07-26):** the urgent mobile containment quickfix and the full lightbox gesture feature are **separate deliveries**. The quickfix shipped as Story 48.1; library adoption for the mature viewer is **not** pre-decided.
- **Architecture (§ Initiative 26, Decisions AX–BA):** new `BrowseCategory` + `ModelBrowseCategory` entities reusing the proven `ModelTag` composite-PK / CASCADE / RESTRICT shape, shipped **atomically with migration `0020_browse_categories`** (`down_revision = "0019_drop_category"`, additive and reversible) because the ORM↔migration parity gate fails on either half alone; additive API — `GET /api/categories`, `GET /api/categories/{slug}`, `GET /api/models?category=<slug>`, `ModelDetail.categories`, admin CRUD + replace-set assignment; tag-aware `q` as a membership disjunct inside the existing `or_()`; the suggestion surface **reuses** the shipped `GET /api/tags?q=` rather than adding an endpoint, resolving group labels from the already-loaded `useTagGroups()` map; lightbox adoption deferred to a scored decision story.
- **Concurrency posture (controller decision, 2026-07-26):** admin category assignment is **replace-set with explicit, auditable last-writer-wins**. LWW **does permit a lost update** — two admins editing from the same snapshot will silently discard the earlier save — and this PRD does not claim otherwise. It is accepted for the current **single-admin** deployment because every write emits an audit row carrying the resulting set, making a lost update recoverable and attributable after the fact. **Named upgrade trigger:** the moment a second concurrent admin editor or an automated/agent writer exists, optimistic concurrency (a `revision` integer or ETag/`If-Match` returning `409` on stale writes) becomes required.
- **Ratification (controller, Laura, 2026-07-26):** after reading the SCP, PRD, architecture and epics artifacts under the operator-approved input packet and the standing `działaj` delegation, the controller **ratifies** (a) re-use of the `/api/categories` path for the new contract and (b) the internal `BrowseCategory*` / `browse_category` identifier convention, and **decides** that a **targeted `bmad-ux` pass runs before Story 50.3, Epic 51 and Epic 52**. **No Ezop sign-off is recorded or implied**, and no second operator question is raised for the already-approved product direction.

### Out of scope (intentional; deferred with named triggers)

- **Restoring anything from the retired taxonomy** — `Model.category_id`, the `category` table contract, the recursive `CategoryTree`, or mandatory categorization. Permanently out of scope; migration `0019_drop_category` is never reverted.
- **Changing `Tag.group_id` to many-to-many** — explicitly rejected by the operator decision above. Trigger: a fresh owner decision only.
- **Automatic category inference or synchronization from tags** — advisory suggestions only, if any. Trigger: demonstrated curation cost at real catalog scale.
- **Multi-category public scope, and a third hierarchy level** — one active scope, depth ≤ 2. Trigger for child-category UI: real catalog distribution demonstrating the need.
- **A new search backend/engine, fuzzy or typo-tolerant matching** — substring matching over existing columns only. Trigger: measured recall failure that substring cannot address.
- **Categories on the anonymous share projection** — `ShareModelView` is unchanged; NFR25-LEAKFENCE-1's negative share-DTO test stays as the fence. Trigger: an explicit product decision, never an incidental addition.
- **Migrating `Viewer3DModal` or other shared `DialogContent` consumers** to the new viewer or to the E48.1 geometry pattern — Story 48.1's recorded residual risk. Trigger: a reported defect or an explicit story.

### Cross-references

- Source SCP: `sprint-change-proposal-2026-07-26-init26-catalog-discovery.md` (direction operator-approved 2026-07-26; SCP text ratified by Laura/controller 2026-07-26 — G26-SCP-RATIFY closed).
- Readiness: `implementation-readiness-report-2026-07-26.md` (native `bmad-check-implementation-readiness`, re-run 2026-07-26 against the corrected artifacts).
- Input packet: `/tmp/3d-portal-init26-correct-course-input.md` (includes the 18-source research registry).
- Architecture: `architecture.md` § Initiative 26 (Decisions AX data model / AY API contract / AZ migration `0020` / BA lightbox adoption procedure).
- Epics: `epics.md` § Initiative 26 (E48 shipped + E49–E54 backlog).
- Sprint status: `_bmad-output/implementation-artifacts/sprint-status.yaml` — `epic-48`/`48-1-*` reconciled to `done`; `epic-49`–`epic-54` + stories seeded at `backlog` by this correct-course per checklist item 6.4.
- Shipped predecessor work: commit `da87e71`; `_bmad-output/implementation-artifacts/spec-e48-1-mobile-fullscreen-containment.md`.
- Preserved history: Initiative 25 FR25 block and its supersede note are unchanged; the forward pointer above distinguishes the retired mandatory single category from the new M:N browse category.
- Gates after the 2026-07-26 controller review: **G26-SCP-RATIFY ✅ closed** (SCP text ratified by Laura/controller 2026-07-26; no Ezop sign-off implied) · **G26-ROUTE-PATH ✅ closed** (`/api/categories` re-use ratified, together with the internal `BrowseCategory*` naming) · **G26-UXGATE ✅ decided** — a **targeted** `bmad-ux` pass runs before Story 50.3, Epic 51 and Epic 52, scoped to the browse nav, mobile Browse surface, scope chip, suggestion dropdown, Filters drawer, admin category/curation screens and lightbox chrome; E49 and Story 53.1 are not blocked by it · **G26-CAT-SET 🔓 open, routed** — the concrete ~6–10 starter categories and their FR26-GOV-1 inclusion criteria are produced by that same UX/taxonomy pass, before Story 49.2 is created · **G26-LIB 🔓 open** (lightbox adoption; needs the Story 53.1 recommendation and physical-Android evidence) · **G26-MIGRATE 🔓 open** (`0020` additive/reversible confirmation at Story 49.1 create-story; explicitly not a destructive gate) · **G26-DEVGO 🔓 open** — planning proceeds now; code on any story starts only after `bmad-create-story` + `bmad-create-story:validate` and the controller confirming that specific ready story under the user's standing initiative authorization.
- Process notes: dependency order is backend/data (E49) → FE data + state + suggestions (E50) → browse IA cutover (E51) → filters + admin curation (E52) → lightbox (E53, independent track) → i18n/a11y/visual/rollout (E54). Apply [[feedback_scp_pre_enumeration_phase]] to each story spec; apply [[reference_web_routetree_regen]] for the new `/categories/$slug` route; visual specs render pl-PL per [[web-visual-tests-render-pl-pl]]. Every `VERIFY-AT-CREATE-STORY` marker in the epic sketches is a mandatory fresh repo-wide trace at story-creation time (standing epic:47 action item).
