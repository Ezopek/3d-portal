...
- **Status:** `review` — implemented 2026-07-29 by native `bmad-dev-story` (**DS**) under a controller-issued `G26-DEVGO`, then repaired and gated by the **controller** after the DS run terminated on `error_max_turns` mid-hygiene (§15). Created and validated 2026-07-29 by native `bmad-create-story` (Create **CS** → Validate **VS**), verdict **PASS** (§13). **NOT `done`:** no commit, no ff-merge, no push, no deploy, no post-deploy smoke has been run (§15.6).
...
### 15.4 What was NOT run, and is not claimed

- `npm run test:visual` **repo-wide** across all four projects. Only the two story-relevant specs were run. AC-36's visual clause is therefore **not** discharged.
- `infra/scripts/check-all.sh` 16/16 — **not run**. AC-36 is **not** discharged.
- **AC-37 / NFR26-DETERMINISM-1** — the 3× consecutive identical vitest **and** pytest counts were **not** produced. The controller reproduced the DS run's 913-test figure **once**. AC-37 is **open**.
- The backend suite (`apps/api` pytest) — not run. Justified by construction (`apps/api/**` is absent from the change set) but that is an expectation, not a measurement.
- `.hermes/run-logs/e51.2-*.log` tees (§7.8) — not produced.

### 15.5 AC ledger at closeout

Satisfied and evidenced: AC-1, AC-2, AC-4…AC-18, AC-19…AC-35 (per the unit/integration/visual suites above and the source reads in §14.3). **AC-3 partially verified** — rules unchanged and every shipped assertion intact, but the "test file unmodified" clause was not measured (completion note 6). **AC-36 not satisfied** (`check-all.sh` and repo-wide `test:visual` unrun). **AC-37 not satisfied** (no determinism triple). AC-38 satisfied by an empty fence set.

### 15.6 Still owed, all controller-owned

`infra/scripts/check-all.sh` 16/16 → repo-wide `npm run test:visual` → the AC-37 determinism triple → `git diff` on `routes/catalog/index.test.ts` (completion note 6) → commit (with `baseline-reviewed: …, Claude Opus 5, 2026-07-29` on all 10 baselines) → ff-only merge → push → `infra/scripts/deploy.sh` → post-deploy smoke → close the three epic-51 `action_items` (T15) → flip `51-2` to `done`.

**No commit, stage, push, merge, deploy, migration, seed, live-DB or network action has been taken.** No deploy has been run.
