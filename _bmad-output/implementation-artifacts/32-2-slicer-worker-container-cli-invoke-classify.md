...
- **AC-12** — DOC ONLY in this repo; `git diff <baseline> -- '*configs*'` empty (zero configs-repo edits, HC2 preserved). External gate PARTIALLY CLOSED by the 2026-06-01 controller deploy: deploy of `1b76e50` to `.190`, configs recipe synced + hotfixed (`979f4a8`/`c25e30e`), real `portal-slicer-worker:0.1.0` image built (OrcaSlicer 2.3.2, `--appimage-extract` R3 spike resolved), and Orca-in-container CLI + `--info` geometry smoke OK. **Residual gate OPEN** = the full PLA/TPU `slice_estimate` job (no vendored profiles under `/mnt/raid/3d-portal-content/slicer` yet) — see blocker.
...
**RESIDUAL BLOCKER (pre-merge, controller-owned) — AC-12 full PLA/TPU slice-estimate-job smoke (NARROWED 2026-06-01 by the controller deploy):** the deploy + real-image + `--appimage-extract` R3 spike + Orca-in-container CLI/geometry-smoke halves of AC-12 are now DONE (see the controller-deploy bullet in Debug Log References): commit `1b76e50` deployed to `.190`; the configs `slicer-worker.yml` recipe synced to `/mnt/raid/configs/...` + durably hotfixed in the configs repo (`979f4a8`, `c25e30e`; configs `origin/main` @ `c25e30e`); a real slicer-capable image `portal-slicer-worker:0.1.0` (OrcaSlicer 2.3.2 layered on `portal-api:0.1.0`, AppImage SHA256-verified) built; the `slicer-worker` overlay running; `/opt/orca/orca --info` over a real in-container tetra STL returns rc 0 + `manifold = yes`. **What REMAINS OPEN:** a FULL PLA/TPU `slice_estimate` job through the resolver → bundle → store → queue path was NOT exercised, because `/mnt/raid/3d-portal-content/slicer` has NO vendored profile/bundle files yet. So the AC-12 out-of-band slice smoke (PLA + TPU fixtures → exit 0 + expected g-code metadata) — verified via the portal-side `ORCA_SMOKE_TEST=1` bench bridge (`test_real_orca_slice_smoke`) — is the residual real slicing/profile-data gate, deferred to Story 32.3 / 32.5 or a final AC-12 slice-job smoke. The worker is NOT "done" until that full slice smoke passes. No merge to main; PR #2 draft/open. No portal-side commit touches `~/repos/configs/*` (HC2).
