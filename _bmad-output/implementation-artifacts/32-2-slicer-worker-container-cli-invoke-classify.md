...
- **Controller deploy to `.190` + AC-12 deploy/image/Orca-in-container CLI+geometry smoke (2026-06-01, status stays `review`):** the controller committed the app-side worker as `1b76e50` (`feat: add slicer worker job`) on branch `feat/E32.2-slicer-worker-container-cli-invoke-classify` and deployed it to `.190` via `infra/scripts/deploy.sh`. This advances the AC-12 gate from "merged-only" to "deployed + real image + CLI/geometry smoke OK", narrowing the residual blocker to the full slice-estimate job (below).
  - **Deploy succeeded end-to-end:** images built/shipped, stack restarted, alembic ran, symbolication verify OK for release `0.1.0+1b76e50`, runbook fingerprint OK. Post-deploy runtime health OK: `/api/health` → `{"status":"ok","version":"0.1.0"}`; containers up (web / api / arq-worker / render worker / redis).
  - **Configs recipe synced + durably hotfixed (HC2 — configs repo, NOT this repo):** `slicer-worker.yml` synced to `/mnt/raid/configs/docker-compose-recipes/workers/slicer-worker.yml`, then hotfixed in the configs repo after a first-start `Settings` validation failure — `979f4a8 fix: wire slicer worker required settings` (wires required `Settings` env: `JWT_SECRET` / TOTP / etc.) + `c25e30e feat: add slicer worker image build recipe` (adds `docker-compose-recipes/workers/slicer-worker.Dockerfile`, default image `portal-slicer-worker:${PORTAL_VERSION}`, `ORCA_BIN` / `SLICER_ORCA_BIN` wiring, docs). configs `origin/main` now at `c25e30e`.
  - **Real slicer-capable image built on `.190`:** `portal-slicer-worker:0.1.0` built from the configs Dockerfile, layering the OrcaSlicer 2.3.2 AppImage on top of `portal-api:0.1.0` (AppImage SHA256 verified during build). The `--appimage-extract` R3 spike thus resolves against a real image.
  - **slicer-worker overlay started:** `docker compose -f docker-compose.yml -f /mnt/raid/configs/docker-compose-recipes/workers/slicer-worker.yml --profile slicer-worker up -d slicer-worker`; container up on `portal-slicer-worker:0.1.0`, logs `Starting worker for 1 functions: slice_estimate` + Redis connected.
  - **Orca-in-container smoke (CLI + geometry — the AC-3/AC-12 bench bridge):** in-container `Settings` load OK, `orca_bin=/opt/orca/orca` exists + executable, `/opt/orca/orca --help` rc 0 identifying `OrcaSlicer-2.3.2`; real geometry smoke — a tiny tetra STL created in-container + `/opt/orca/orca --info /tmp/tetra.stl` → rc 0, `manifold = yes`, dimensions/volume printed.
  - **Residual real data/profile gate (why status stays `review`):** `/mnt/raid/3d-portal-content/slicer` currently has NO vendored profile/bundle files, so a FULL PLA/TPU `slice_estimate` job through the resolver → bundle → store → queue path was NOT exercised. The AC-12 out-of-band slice smoke (PLA + TPU fixtures → exit 0 + expected g-code metadata; portal-side `ORCA_SMOKE_TEST=1` `test_real_orca_slice_smoke` bench bridge) therefore REMAINS OPEN — carried as the residual real slicing/profile-data gate for Story 32.3 / 32.5 or a final AC-12 slice-job smoke, whichever the story sequence reaches first.
  - **No merge to main; PR #2 remains draft/open.** Commit `1b76e50` is on the story branch only.

- **Controller AC-12 RESIDUAL CLOSURE — full PLA/TPU `slice_estimate` job through the real `arq:slicer` queue (2026-06-01, status `review` → `done`):** the last-open half of AC-12 (the full resolver → bundle → store → queue → worker slice smoke, previously blocked because `/mnt/raid/3d-portal-content/slicer` carried no vendored profile/bundle files) is now CLOSED with controller-verified runtime evidence on `.190`. This satisfies the sole remaining gate that had kept the story in `review`.
  - **Vendored profile export installed (the missing data dependency):** an Orca profile export from the Fenrir/WSL bench (source `192.168.2.100`, `/mnt/c/Users/ezope/AppData/Roaming/OrcaSlicer`) was installed at `/mnt/raid/3d-portal-content/slicer/vendored` on the `.190` runtime host. The export used the AI user profiles where compatible — PLA: process `"AI 0.20mm (0.4 nozzle) - MicroSwiss HF"` + filament `"AI Rosa3D PLA Starter"`; TPU: process `"AI 0.20mm TPU - FlowTech"` + filament `"AI Rosa3D Flex 96A Black"`. The **machine** resolved to the compatible base `"Creality K1 Max (0.4 nozzle)"` because Orca rejected the AI machine profile with `"process not compatible with printer"` — a **data-compatibility adjustment in the vendored profile set, NOT a code bypass** (the worker/resolver code path is unchanged; AC-13 scope fence intact). Export sanitization converted Orca `"inherits"` → resolver `"inherit"` and stripped `print_host`/secret-ish profile fields before runtime vendoring.
  - **Worker runtime under test:** container `3d-portal-slicer-worker-1`, image `portal-slicer-worker:0.1.0`, OrcaSlicer `2.3.2`. STL content hash for the smoke = `fe3b546eddb0ced0c9988dce8d69cc3679fe3f82e324f62e4b7dc138912febd0`.
  - **Real `arq:slicer` queue smoke — both material classes returned `status ok`:**
    - **PLA** bundle `25b03be589a43e220ae0dd0452f2dca636ce01865af7a2c87e8dfe8708c5d501`, `job_id slice:fe3b546eddb0ced0c9988dce8d69cc3679fe3f82e324f62e4b7dc138912febd0:25b03be589a43e220ae0dd0452f2dca636ce01865af7a2c87e8dfe8708c5d501` → result `status ok`, `reason None`, `warnings []`, `manifold True`, `orca_version 2.3.2`, `slice_wall_ms 455`.
    - **TPU** bundle `d90a4adb9865bf800686474457a13abf483b72bbe511eafd9b0925aa5d576ca3`, `job_id slice:fe3b546eddb0ced0c9988dce8d69cc3679fe3f82e324f62e4b7dc138912febd0:d90a4adb9865bf800686474457a13abf483b72bbe511eafd9b0925aa5d576ca3` → result `status ok`, `reason None`, `warnings []`, `manifold True`, `orca_version 2.3.2`, `slice_wall_ms 673`.
  - **Parse-and-discard verified (AC-5 at real runtime):** the returned temp g-code refs under `/tmp/portal-slice-*` no longer existed after job completion — zero durable g-code retention, confirmed on the live container, not just the mocked-runner unit path.
  - **Worker log:** showed `arq:slicer` jobs started and completed with `status ok` for both bundle hashes.
  - **Net AC-12 verdict:** all three AC-12 halves are now DONE — (a) configs `slicer-worker.yml` recipe merged + synced/deployed/hotfixed on `.190` (`979f4a8`/`c25e30e`); (b) real slicer-capable image `portal-slicer-worker:0.1.0` built + `--appimage-extract` R3 spike resolved + Orca-in-container CLI/`--info` geometry smoke OK; (c) the full out-of-band PLA/TPU `slice_estimate` job smoke (exit 0 + valid `SliceOutcome`, `manifold True`, parse-and-discard) PASSED. No app-side code change accompanies this closure (runtime/data only); `~/repos/configs/*` untouched from this repo (HC2). PR #2 still draft/open on branch `feat/E32.2-slicer-worker-container-cli-invoke-classify` — commit/ff-merge to `main` + PR finalize remain controller-owned next steps, distinct from the BMAD story `done` flip.
  - **Carried-forward MINOR (non-blocking, does NOT gate `done`):** review finding #5 (broad `is_profile_rejection()` markers in `cli.py`) was deferred to observing a REAL Orca profile-load-rejection stderr; this PLA/TPU smoke succeeded (no rejection path exercised), so the narrowing remains an open low-blast-radius minor (both branches still yield a typed `failed`, never a silent zero — FR20-FAILURE-1 preserved). It can be picked up opportunistically in Story 32.3/32.5; not a Story 32.2 blocker.
...
- **AC-12** — DOC ONLY in this repo; `git diff <baseline> -- '*configs*'` empty (zero configs-repo edits, HC2 preserved). External gate now **FULLY CLOSED** (controller-verified, 2026-06-01): (a) deploy of `1b76e50` to `.190` + configs recipe synced + hotfixed (`979f4a8`/`c25e30e`); (b) real `portal-slicer-worker:0.1.0` image built (OrcaSlicer 2.3.2, `--appimage-extract` R3 spike resolved) + Orca-in-container CLI + `--info` geometry smoke OK; (c) **the full out-of-band PLA/TPU `slice_estimate` job smoke PASSED** — vendored profiles installed at `/mnt/raid/3d-portal-content/slicer/vendored`, real `arq:slicer` queue run for STL `fe3b546e…` over PLA bundle `25b03be5…` (`status ok`, `manifold True`, `slice_wall_ms 455`) and TPU bundle `d90a4adb…` (`status ok`, `manifold True`, `slice_wall_ms 673`), `orca_version 2.3.2`, parse-and-discard verified (`/tmp/portal-slice-*` gone post-job). No residual AC-12 gate remains — see the AC-12 closure bullet in Debug Log References.
...
**RESIDUAL BLOCKER — RESOLVED 2026-06-01 (controller-verified; AC-12 fully closed):** the previously-open AC-12 half (the FULL PLA/TPU `slice_estimate` job through the resolver → bundle → store → queue path, blocked because `/mnt/raid/3d-portal-content/slicer` had no vendored profile/bundle files) has now PASSED on the `.190` runtime. Vendored profiles were installed at `/mnt/raid/3d-portal-content/slicer/vendored` (Orca export from the Fenrir/WSL bench `192.168.2.100`, sanitized `inherits`→`inherit` + stripped `print_host`/secret-ish fields; machine resolved to base `"Creality K1 Max (0.4 nozzle)"` as a data-compatibility adjustment, not a code bypass), and a real `arq:slicer` queue smoke ran in container `3d-portal-slicer-worker-1` (`portal-slicer-worker:0.1.0`, OrcaSlicer 2.3.2) over STL `fe3b546e…`: PLA bundle `25b03be5…` → `status ok`/`manifold True`/`slice_wall_ms 455` and TPU bundle `d90a4adb…` → `status ok`/`manifold True`/`slice_wall_ms 673`, both `reason None`/`warnings []`/`orca_version 2.3.2`; parse-and-discard verified (`/tmp/portal-slice-*` gone after job); worker log showed both `arq:slicer` jobs started + completed `ok`. **All three AC-12 halves (deploy/image/R3-spike, CLI/geometry smoke, full PLA/TPU slice-job smoke) are DONE.** The earlier deploy/image/CLI-geometry halves remain as recorded: commit `1b76e50` deployed to `.190`; configs `slicer-worker.yml` recipe synced + hotfixed (`979f4a8`, `c25e30e`; configs `origin/main` @ `c25e30e`); real `portal-slicer-worker:0.1.0` image (OrcaSlicer 2.3.2 layered on `portal-api:0.1.0`, AppImage SHA256-verified) built; `--appimage-extract` R3 spike resolved; `/opt/orca/orca --info` over an in-container tetra STL rc 0 + `manifold = yes`. With code-review already passed (no blocker / no important) and the sole AC-12 gate now satisfied, the story flips `review` → `done`. The deferred MINOR (broad `is_profile_rejection()` markers — review finding #5) is carried forward as a low-blast-radius non-blocker (typed `failed` either branch, never a silent zero) for opportunistic narrowing in 32.3/32.5, not a Story 32.2 blocker. **Distinct controller-owned follow-ups (NOT part of the BMAD `done` flip):** commit/ff-merge to `main` + PR #2 finalize remain pending per AGENTS.md § Branching; no portal-side commit touches `~/repos/configs/*` (HC2).
...
| 2026-06-01 | Controller deploy to `.190` + AC-12 partial closure (status stays `review`). Committed the app-side worker as `1b76e50` (`feat: add slicer worker job`) on the story branch and deployed to `.190` via `infra/scripts/deploy.sh`: images built/shipped, stack restarted, alembic ran, symbolication verify OK release `0.1.0+1b76e50`, runbook fingerprint OK; `/api/health` → `{"status":"ok","version":"0.1.0"}`, containers up (web/api/arq-worker/render/redis). Configs `slicer-worker.yml` synced to `/mnt/raid/configs/docker-compose-recipes/workers/slicer-worker.yml` + durably hotfixed in the configs repo (`979f4a8` wire required `Settings` env after a first-start validation fail; `c25e30e` add `slicer-worker.Dockerfile` + default image `portal-slicer-worker:${PORTAL_VERSION}` + `ORCA_BIN`/`SLICER_ORCA_BIN` + docs; configs `origin/main` @ `c25e30e`). Built real `portal-slicer-worker:0.1.0` on `.190` (OrcaSlicer 2.3.2 AppImage SHA256-verified, layered on `portal-api:0.1.0`) → `--appimage-extract` R3 spike resolved; started the `slicer-worker` overlay (`Starting worker for 1 functions: slice_estimate`, Redis connected). Orca-in-container smoke: `Settings` OK, `orca_bin=/opt/orca/orca` exists+executable, `--help` rc 0 `OrcaSlicer-2.3.2`; real geometry `--info` over an in-container tetra STL rc 0 `manifold = yes`. AC-12 deploy/image/R3-spike/CLI+geometry-smoke halves DONE; residual OPEN = full PLA/TPU `slice_estimate` job through resolver→bundle→store→queue NOT exercised (no vendored profiles under `/mnt/raid/3d-portal-content/slicer`) — deferred to 32.3/32.5 or a final AC-12 slice-job smoke. No merge to main; PR #2 draft/open. Status stays `review`. |
...
| 2026-06-01 | **AC-12 residual gate CLOSED → status `review` → `done` (controller-verified runtime evidence on `.190`).** The last-open AC-12 half — the full PLA/TPU `slice_estimate` job through the real `arq:slicer` queue — PASSED. Vendored Orca profiles installed at `/mnt/raid/3d-portal-content/slicer/vendored` (export from Fenrir/WSL bench `192.168.2.100`; sanitized `inherits`→`inherit`, stripped `print_host`/secret-ish fields; machine resolved to base `"Creality K1 Max (0.4 nozzle)"` as a data-compatibility adjustment after Orca rejected the AI machine profile — NOT a code bypass, AC-13 fence intact). Real `arq:slicer` smoke in container `3d-portal-slicer-worker-1` (`portal-slicer-worker:0.1.0`, OrcaSlicer 2.3.2) over STL `fe3b546e…`: PLA bundle `25b03be5…` → `status ok`/`reason None`/`warnings []`/`manifold True`/`slice_wall_ms 455`; TPU bundle `d90a4adb…` → `status ok`/`manifold True`/`slice_wall_ms 673`; `orca_version 2.3.2`. Parse-and-discard verified at runtime (`/tmp/portal-slice-*` gone post-job); worker log shows both jobs started + completed `ok`. All three AC-12 halves now DONE; code-review already passed (no blocker / no important). Story flips to `done`. Deferred MINOR (#5 `is_profile_rejection` breadth) carried forward as a non-blocking low-blast-radius item for 32.3/32.5. Runtime/data only — no app-side code change; `~/repos/configs/*` untouched (HC2). Controller-owned follow-ups (commit/ff-merge to `main` + PR #2 finalize) remain pending, distinct from the BMAD `done` flip. |
