...
- source_spec: `_bmad-output/implementation-artifacts/spec-46-2-rename-merge-move.md`
  summary: `TagGroupsPage.reorder` swaps two adjacent groups' `position` via two independent PATCHes with no rollback — if the first lands and the second fails, both groups share a `position` (order becomes slug-tie-broken); an equal-position swap is also a silent no-op that still shows a success toast.
  evidence: Low consequence (group display order only, self-heals on the next successful reorder or refetch, and only triggers on a mid-swap server error), and the spec's Design Notes explicitly pre-accepted the tradeoff. The robust fix is a single atomic backend reorder endpoint (accepting an ordered id list), which is out of scope for this frontend-only story. Confirmed by both adversarial reviewers (Story 46.2 review pass).
