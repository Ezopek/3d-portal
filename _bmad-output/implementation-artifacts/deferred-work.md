...
**STATUS — IMPLEMENTED (2026-06-03, bmad-quick-dev).** The poll-diff event source — the only piece that was left — shipped on branch `feat/SPOOL-EVT-1-spoolman-change-source` (code-side; controller/Laura owns the merge + `.190` deploy gate). Spec: `spec-spool-evt-1-spoolman-change-source.md`. What shipped, against the 4-step plan above:
...
**Scope fences preserved (unchanged):** NO catalog→`stl_hash` ingestion (EST-INGEST-1 still deferred), NO `POST /api/estimates/recompute` (EST-RECOMPUTE-1 still deferred), NO UI, NO second Spoolman poll. The end-to-end "Spoolman change auto-updates estimates" now works **for estimate keys that have been resolved+persisted through the SPOOL-PREQ-1 attribution index** — it does NOT auto-derive estimates for catalog parts that were never sliced (that needs EST-INGEST-1). A missing attribution record or an as-yet-uncomputed bundle is a no-dispatch, by design.
...
**STATUS: PROMOTED + IMPLEMENTED (2026-06-03, branch `feat/EST-RECOMPUTE-1-recompute-now`, pending Laura review/gates/merge/deploy).** Promoted now that EST-INGEST-1 shipped (catalog `ModelFileRead` carries real per-part `sha256`; production dry-run found 525 STL parts ready for ingestion), creating the real user need to (re)queue estimates from the UI. What landed:
