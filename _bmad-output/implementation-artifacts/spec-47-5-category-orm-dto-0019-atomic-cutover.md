...
- [x] [Review][Patch] **Evidence-provenance gap for the committed tree (medium; NFR25-DETERMINISM-1 / §12 honest-evidence)** — §15 Debug Log References point at `.hermes/run-logs/check-all-20260723_*-e47-5.log` + `determinism-e47-5.log`, but the on-disk logs (bmad-loop run be53 worktree, 06:08–06:21) validate a *divergent uncommitted second-attempt tree* (73 files differ from `98246d7`; its check-all shows 1742/3, not the claimed 1745/3). Committed-tree evidence survives only as collapsed TUI capture (`logs/…dev-1.log:417135` — determinism run 1 = 1745/3; check-all tee started but its artifact was destroyed by the loop reset). Repair: re-run `check-all.sh` + the 3× determinism battery against `98246d7` (rescue branch) and tee fresh logs before merge. Mitigation already in hand: this review independently re-ran all three suites once on `98246d7` — apps/api **1745 passed/3 skipped**, apps/web **785 (136 files)**, workers/render **21** — all matching the commit claims; visual 522/30 confirmed only for the attempt-2 tree. **RESOLVED (2026-07-23, re-review): fresh evidence now belongs to the exact final HEAD `4636d88a6c20a48411c650d38f32cfdfc5c83fa9` — `.hermes/run-logs/check-all-e47-5-final2-4636d88-20260723_071245.log` ends 16/16 all green (apps/api 1747 passed/3 skipped — up from 1745 by the 2 new retired-route negative tests; apps/web vitest 785; workers/render 21; visual 522 passed/30 skipped); `.hermes/run-logs/determinism-e47-5-final-4636d88-20260723_072322.log` opens with `HEAD=4636d88a6c20a48411c650d38f32cfdfc5c83fa9` and records identical triples (apps/api 1747/3 ×3, apps/web 785 ×3, workers/render 21 ×3) ending `DETERMINISM_DONE`.**
...
#### Re-review record (CR-2, focused)

<!-- Native BMAD code re-review (CR, iteration 2), 2026-07-23, fresh context, reviewer: Claude Opus 4.8 (claude-opus-4-8) — the reviewing agent signs as itself. Scope: focused on the repair diff 98246d7...4636d88 + fresh final evidence; unrelated history not re-reviewed beyond regression context. Verdict: APPROVE. -->

- Repair diff `98246d7...4636d88` inspected file-by-file (9 files, +78/−13, docs/tests/scripts only — no production code, no migrations). Every open Decision/Patch finding verified resolved against the actual diff: `cutover-smoke.sh` scenario 5 re-pointed to live auth-protected `/api/tags` preserving the exact anonymous-external default-deny 401 purpose (comment documents the re-point); `docs/operations.md` (112, 457-461, 606-617) and `docs/architecture.md` (25, 34) current surfaces cut over with historical claims honestly annotated (43-categories row + 2026-05-21 curl verification kept and dated as legacy/retired by E47.5); `audit-six-scenarios.sh:673` High-002 comment clarified then-live/retired; direct retired-route negatives `test_retired_taxonomy_read_route_is_gone` (GET → 404) + `test_retired_taxonomy_admin_crud_route_is_gone` (PATCH → 404/405) added in `test_openapi_agent_surface.py` with runtime-assembled literals (§11 grep contract holds — re-verified 0 disallowed hits); `AddModelForm.test.tsx` structural `getAllByRole("combobox")` `toHaveLength(2)` guard added; `test_2fa_schema.py` count test renamed truthfully; `test_bootstrap_agent.py` slug now per-run unique with an honest comment.
- Evidence provenance closed at exact final HEAD `4636d88a6c20a48411c650d38f32cfdfc5c83fa9`: check-all 16/16 all green (API 1747/3, web 785, workers 21, visual 522 passed/30 skipped) and determinism triples identical ×3 with `DETERMINISM_DONE` — exact log paths in the resolved finding above.
- Mechanical re-confirmation this pass: `alembic heads` → exactly `0019_drop_category (head)`; §11 BE residual greps 0 hits (apps/api/app, apps/api/tests, apps/api/scripts, workers/render — venv noise excluded); FE/worker residual hits all within §11 allowed exceptions (slicer material/reason taxonomy, Sentry breadcrumb kwargs, `main.py:40` comment heading); runbook `category` hits 0. No live network/DB touched.
- Independent verdicts on record (supplied factual record, not this reviewer's own actions): Aider full base cutover `98246d7` APPROVE; Aider focused repair `4636d88` APPROVE; Laura/controller independently inspected before/after/diff contact sheets for all 28 changed/added visual baselines and recorded visual ACCEPT in the controller session. Original commit sign-offs attributed Claude Opus 4.8; the prior native review (CR-1) verified dev-log provenance.
- Remaining [Defer] items stand as deferred (unchanged classification). **Final verdict: APPROVE.** Spec status set to `done` per repo precedent for code-review-complete execution specs (44.x–47.3). Sprint-status story/epic NOT flipped — pre-deploy backup, ff-merge, deploy, and controller closeout remain pending (controller/operator-owned, story §12).
