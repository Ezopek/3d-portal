...
@router.get(
    ...
    description=(
        ...
        "a model matching by both its own name and a tag name is returned exactly once, "
        "counted once in `total`. The tag half case-folds non-ASCII letters too, so "
        "`łazienka` matches a tag named `Łazienka`; the model-name half still folds "
        "ASCII only, a pre-existing limitation this story did not widen), "
        "`external_url` (exact match against any of the "
