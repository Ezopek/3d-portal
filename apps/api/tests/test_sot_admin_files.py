...
def test_stl_upload_calls_matrix_load_active_matrix(client, monkeypatch):
    """AC-8 (35.6): uploading an STL file triggers load_active_matrix from the matrix hook."""
    called = []

    def _fake_load_active_matrix(*args, **kwargs):
        called.append(True)
        return []
    ...
    files, data = _multipart(b"solid hook\nendsolid", "hooktest.stl", "stl")
    client.cookies.set("portal_access", admin_token(admin_id))
    r = client.post(f"/api/admin/models/{model_id}/files", files=files, data=data)
    assert r.status_code == 201, r.text
    assert called, "load_active_matrix should have been called by the matrix hook"
...
def test_stl_upload_matrix_hook_exception_does_not_500(client, monkeypatch):
    ...
    monkeypatch.setattr("app.modules.slicer.matrix_backfill.load_active_matrix", _raise)
    ...
    files, data = _multipart(b"solid safe\nendsolid", "safe.stl", "stl")
    client.cookies.set("portal_access", admin_token(admin_id))
    r = client.post(f"/api/admin/models/{model_id}/files", files=files, data=data)
    assert r.status_code == 201, r.text


def test_non_stl_upload_does_not_call_matrix_hook(client, monkeypatch):
    """AC-8 (35.6): non-STL uploads (photo/source) must NOT trigger the matrix hook."""
    called = []
    ...
    files, data = _multipart(b"\x89PNG\r\n\x1a\n", "photo.png", "image")
    client.cookies.set("portal_access", admin_token(admin_id))
    r = client.post(f"/api/admin/models/{model_id}/files", files=files, data=data)
    assert r.status_code == 201, r.text
    assert not called, "matrix hook must not fire for non-STL uploads"
