...
async def test_create_returns_token_and_persists(service):
    token = await service.create(model_id="001", expires_in_hours=24, created_by=7)
    assert len(token.token) >= 16
    assert token.model_id == "001"
    assert token.created_by == 7
...
async def test_revoke_removes_token(service):
    token = await service.create(model_id="002", expires_in_hours=1, created_by=1)
...
async def test_list_active_includes_all_tokens(service):
    t1 = await service.create(model_id="001", expires_in_hours=1, created_by=1)
    t2 = await service.create(model_id="002", expires_in_hours=1, created_by=1)
...
async def test_ttl_zero_hours_is_rejected(service):
    with pytest.raises(ValueError):
        await service.create(model_id="001", expires_in_hours=0, created_by=1)
