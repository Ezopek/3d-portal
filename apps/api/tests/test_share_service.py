...
_MODEL_A = uuid.UUID("00000000-0000-0000-0000-00000000aaaa")
_MODEL_B = uuid.UUID("00000000-0000-0000-0000-00000000bbbb")
...
async def test_create_returns_token_and_persists(service):
    token = await service.create(model_id=_MODEL_A, expires_in_hours=24, created_by=_USER_7)
    assert len(token.token) >= 16
    assert token.model_id == _MODEL_A
    assert token.created_by == _USER_7

    fetched = await service.resolve(token.token)
    assert fetched is not None
    assert fetched.model_id == _MODEL_A
...
async def test_revoke_removes_token(service):
    token = await service.create(model_id=_MODEL_B, expires_in_hours=1, created_by=_USER_1)
...
async def test_list_active_includes_all_tokens(service):
    t1 = await service.create(model_id=_MODEL_A, expires_in_hours=1, created_by=_USER_1)
    t2 = await service.create(model_id=_MODEL_B, expires_in_hours=1, created_by=_USER_1)
...
async def test_ttl_zero_hours_is_rejected(service):
    with pytest.raises(ValueError):
        await service.create(model_id=_MODEL_A, expires_in_hours=0, created_by=_USER_1)
